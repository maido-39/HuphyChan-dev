...
- **6색 선** = 위상별 평균(Fx/Fy/Fz·Mx/My/Mz), **음영** = 주기간 변동(p5-p95).
- **phased 버전**(권장): 보행 위상 표시 —
  - 🟢 **stance**(발이 지면 접지) / 🔵 **swing**(다리 흔듦) 음영
  - 점선 = **toe-off**(발 뗌, stance→swing 전이) · **opposite-contact**(반대발 접지 = double-support 시작)
