...
| ★★★ | `max_delta_deg` 는 속도가 아니라 **힘**을 자름 (이름·주석과 다름) | `safety/guards.py:86` | **고쳐서 드림** (가지 `rate-limit-from-datasheet`) |
| ★★★ | 고장값을 반대 순서로 읽음 | `motors/robstride/codec/mit.py` | **원본이 이미 고침** (`b240a8f`) |
| ★★★ | 온도 칸의 위쪽 4비트를 온도로 계산 (33도 → 3309.8도) | `motors/robstride/codec/mit.py` | **원본이 이미 고침** (`b240a8f`) |
...
### 고쳐서 드립니다 — 가지 `rate-limit-from-datasheet`

세 가지를 나눴습니다. **설정하지 않은 로봇은 예전과 완전히 동일하게 동작합니다.**

**① 속도 제한을 따로 만들었습니다 (`guards.clamp_rate`).** 기준점이 **직전 명령**입니다.
이것 하나가 결정적입니다:

- 명령은 모터가 뒤처지든 말든 **보장된 속도로 계속 나아갑니다.**
- 오차(명령 − 측정)가 **자유롭게 커지므로** 따라잡거나 부하를 버틸 힘을 온전히 씁니다.

지금 방식은 기준점이 측정이라 **스스로를 조입니다**: 모터가 뒤처지면 측정이 안 나아가고 →
명령도 안 나아가고 → 오차가 안 쌓이고 → 힘이 안 커집니다. 느릴수록 더 느려집니다. 위 표의
"이론 300도/s, 실측 49도/s"가 그 결과입니다.

**② `max_delta_deg` 는 원래 자리로 돌렸습니다.** 스키마의 기본값 50도와 그 설명("계산이
튀었을 때 그 이상은 안 나가게 막는 상한")대로 **폭주 상한**입니다. 속도 제한으로 쓰지 말라는
경고를 docstring 에 적었고, 반증된 "클리핑 = 속도 제한이다" 주석은 정정했습니다. 두 클리핑은
`clips_rate` / `clips_jump` 로 **따로 셉니다** — 어느 쪽이 자르고 있는지가 곧 "왜 느린가"의
답이라 한 칸에 합치면 안 됩니다.

**③ 한도를 초당 각도로 적습니다.** `SafetyConfig.max_vel_deg_s` (모터별은
`max_vel_deg_s_by_motor`). 루프가 `max_step_deg(모터, hz)` 로 자기 주기에 맞춰 환산합니다.
"한 주기에 몇 도"로 적으면 제어 주기를 바꿨을 때 실효 속도가 조용히 따라 변합니다.

```python
# src/huphy/safety/guards.py — 새로 추가
def clamp_rate(target_deg, prev_cmd_deg, max_step_deg):
    """직전 **명령**에서 max_step 이상 벗어나지 않게 자른다."""
    if prev_cmd_deg is None:
        return float(target_deg), False      # 호출부가 이때 측정값으로 다시 잡는다
    delta = float(target_deg) - float(prev_cmd_deg)
    limit = abs(float(max_step_deg))
    if delta > limit:  return prev_cmd_deg + limit, True
    if delta < -limit: return prev_cmd_deg - limit, True
...
RS03·RS04 매뉴얼, **출력단** 기준입니다. 단위 환산은 `rpm × 6 = 도/s`,
`rpm × 2π/60 = rad/s` 입니다 — 여기서 틀리면 60배가 조용히 어긋납니다.

| 조건 | rpm | rad/s | 도/s |
|---|---|---|---|
| 무부하 | 200 ± 10% | 20.94 | 1200 |
| RS03 정격 (hip_yaw) | 180 | 18.85 | **1080** |
| RS04 정격 (knee) | 167 | 17.49 | **1002** |

기준은 **정격**이어야 합니다. 무부하 200 rpm 은 힘을 전혀 내지 않을 때만 나오는 값이라 어떤
부하에서도 유지되지 않습니다. 벤치에서는 그 일부만 씁니다.

### 명령 주기와 제어 주기가 다른 것도 함께 처리했습니다

명령은 **50 Hz** 로 도착하는데 제어 루프는 **100 Hz** 이고, 두 시계가 따로 돌기 때문에 한
목표가 정확히 2번이 아니라 **1~3번** 쓰입니다. 그래서 한 주기 몫을 선언된 주기가 아니라
**실측 경과 시간**으로 계산합니다. 어느 쪽 주기를 바꿔도 실효 속도가 변하지 않습니다.

예외 두 가지를 뒀습니다:

- 경과 시간을 **50 ms 로 자릅니다.** 루프가 한 번 멈췄다 돌아오면 경과가 크게 잡히고 그만큼
  큰 한 걸음이 허용되는데, 그건 반대 방향의 사고입니다.
- 첫 명령과 **끊김(0.2초) 뒤 첫 명령**에는 100 Hz 를 가정합니다. "모르면 걸지 않는다"로 두면
  하필 **가장 위험한 그 한 번이 제한 없이** 나갑니다.

### ★ 함께 들어간 안전 규칙 하나 — 끊김 뒤 설정점 재시딩

명령이 0.2초 넘게 끊겼다 재개되면 직전 명령을 **측정값으로 다시 잡습니다.** 안 그러면
설정점이 관절에서 멀리 떨어진 채 남아 있다가 **재개 순간 그 간격을 통째로 명령합니다.**

저희 벤치에서 2026-09-07 에 의도치 않은 이동이 두 번 났는데(40도·44도, 그리고 10도) **셋 다
재개 첫 패킷**이었습니다. 기준점을 명령으로 옮기면 이 규칙이 **선택이 아니라 필수**가 됩니다 —
지금 방식은 기준이 측정이라 이 성질을 공짜로 갖고 있었고, 그것이 지금 방식의 유일한 장점입니다.

### 설정 예

```yaml
safety:
  max_delta_deg: 50.0        # 폭주 상한. 속도 제한으로 쓰지 말 것
  max_vel_deg_s: 60.0        # 진짜 속도 제한
  max_vel_deg_s_by_motor:
    knee: 60.0               # RS04 정격 1002 도/s 의 6%
    hip_yaw: 60.0            # RS03 정격 1080 도/s 의 5.6%
```

첫 값을 낮게 잡는 이유: 지금 3도 방식은 힘이 잘려 **실측 49 도/s** 밖에 안 나오지만, 새
구조에서 60 도/s 를 넣으면 **정말로 60 도/s 가, 온전한 힘으로** 나옵니다.

시험 20개를 함께 넣었습니다(`tests/test_rate_limit.py` 15개, `tests/test_leg.py::TestRateLimit`
5개). 전체 1094개 통과합니다.
...
| 가지 | 끝 커밋 | 내용 |
|---|---|---|
| ~~`fix-fault-byte-order`~~ | `bc7030e` | ~~1-2(고장값 순서) + 1-3(온도 위쪽 4비트)~~ — **이제 필요 없습니다.** `biped` 가 `b240a8f`("매뉴얼과 어긋난 MIT 프레임 해석 세 건")로 두 건을 **독립적으로 고쳤습니다.** 결과가 같습니다: 고장값은 작은 자리 먼저, 온도는 아래 12비트만. 저희 것은 남겨 두되 **합치지 마십시오** |
| `bench-partial-rig` | `2e46a93` | 위 가지 전부 + 저희 벤치 설정 파일 하나. **라이브러리 쪽으로 새로 들어가는 내용이 없습니다** |
| `rate-limit-from-datasheet` | `b67366d` | **지금은 이것만 보시면 됩니다.** 2-2(속도 제한을 직전 명령 기준 램프로 분리, `max_delta_deg` 는 폭주 상한으로 환원, 한도를 초당 각도로). 시험 20개 포함, 최신 `biped`(`b4582b8`) 합친 뒤 전체 **1116개 통과** |

**원본은 건드리지 않았습니다** — 2026-09-07 올린 뒤 확인한 값: `main` = `133855f`(변동 없음),
`biped` = `b4582b8`. 새 가지만 추가했습니다.

`biped` 는 저희가 처음 분기한 `cd6b8ac` 에서 세 커밋 전진해 있었습니다. 그중 `b240a8f` 가
1-2·1-3 과 같은 곳을 고쳤으므로, `rate-limit-from-datasheet` 에 최신 `biped` 를 **합친 뒤**
시험을 다시 돌려 1116개 통과를 확인하고 올렸습니다. 저희 수정과 원본 수정이 겹치는 곳은
없습니다(저희는 `safety/`, `config/`, `robots/leg.py`, 원본은 `motors/`, `scripts/`).
