...
|---|---|---|---|---|---|
| hip_pitch | −125 / +30 | **−120 / +26** | 양쪽 다 `hip_pitch_link ~ base_link` (힙 요크가 골반에) | −120 / **+25** | 신전 25 사용자 지정(2026-08-23), 금속은 +26 |
| hip_roll | −45 / +25 | **−86 / +30** | 양쪽 다 `hip_pitch_link ~ hip_roll_link` | **−85** / +25 | 외전 85 사용자 지정, 금속은 −86. 내전 +25는 gen21 판정(2026-07-13) |
| hip_yaw | −50 / +50 | **−47 / +51** | 양쪽 다 `hip_roll_link ~ thigh_link` | **±45** | 사용자 지정, 금속은 −48 / +52 |
...
### §3a-정정 (2026-08-23) — hip_roll 외전 −45는 근거가 없었다

표의 초판은 hip_roll 외전 −45°를 "설계 캡 유지"라고 적었다. **근거가 없다.** −45는 옛 모델
`pygmalion.xml`의 `range="-0.785398 …"`을 그대로 물려받은 숫자이고, docs 어디에도 외전 45°에 대한
판단 기록이 없다(문서화된 것은 내전 +25뿐). 사용자가 처음부터 "hip pitch roll yaw knee 다 안 맞다"고
지목한 값을 그대로 두고 이름만 "설계 캡"으로 붙인 셈이다.

근본 원인은 측정 쪽에도 있었다. hip_roll 스윕의 탐색 범위가 −70°에서 끝나 **금속에 닿기 전에
멈췄고**, 결과가 "−70(탐색끝)"으로 나왔는데 이를 한계처럼 읽었다. 범위를 −120°까지 넓혀 다시 재니
**외전 하드스톱은 −86°** 다. 사용자 지정값 −85°는 그 1° 안쪽이다.

같은 실수를 막기 위해: 탐색 span 안에서 아무것도 안 닿으면 그 값은 **한계가 아니라 "탐색 한계"**로
보고되고, `plot_rom.py`가 파란 막대를 span 끝까지 그린다(waist_yaw·shoulder_pitch가 그런 경우다).
...
## §4b 정정 (2026-08-22 저녁) — 꺼진 전구는 억제가 아니다

조립 뷰어를 만들다 사용자가 "CenterParts 나사가 없다"를 지적해 드러난 결함이다. **질량 모델에도
같은 결함이 있었다.**

`bodies.json`의 `live`(light bulb) 플래그를 "이 부품은 로봇에 없다"로 해석했는데, 두 가지가 섞여
있었다:

| 숨겨진 것 | 바디 | 질량 | 판정 |
|---|---|---|---|
| `ArmR_fullDoF` · `Actuators-NotUse` · `Human_Solid ... (REF)` | 25 | **671.044 kg** | 진짜 대체 설계 → 제외 |
| `CenterParts` 내부 나사 | 69 | **0.201 kg** | 부품을 보려고 꺼둔 실물 → **포함해야 함** |

이제 **분기 이름**으로 가른다 (`NotUse` / `fullDoF` / `REF`). 덤프 대조로 검증했다: 이 판별식은
숨겨진 바디 25개(671.0 kg)에 맞고 **살아 있는 바디에는 0개** 맞으며, 판별식에 안 걸리는 숨겨진
바디는 **전부** 나사다.

- 골반 **5.166 → 5.367 kg**, 전체 로봇 **42.098 → 42.299 kg**
- 나사는 CAD x 기준 좌 41 / 중앙 2 / 우 26으로 **양측 모델링**돼 있어 미러링 없이 그대로 센다

부수적으로, 경로에 그룹이 없어 링크를 못 정하던 패스너 66개(루트 레벨 느슨한 오커런스 21개 +
`ANKLE_SPLIT` 45개)는 **최근접 링크 메시**로 배정했다 — 거리 2 mm 수준이라 모호하지 않다.

**교훈: 뷰어에서 "안 보인다"는 신고가 질량 모델의 오류를 찾아냈다.** 시각화는 장식이 아니라 검산이다.
