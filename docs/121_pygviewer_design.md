...
**10b-1. 실기 검증 중 발견·수정한 게이트 자체의 결함 (2026-09-04, docs/123 §10.2b)**: 위
드리프트 검사 (b)가 "sync가 적용한 목표값"(클립 후)을 기준선으로 삼고 있었는데, 클립된
관절은 애초에 그 값이 실측값과 다른 것이 정상이라 실물이 전혀 안 움직여도 매번 거부되는
자기모순이 있었다(벤치 실측: L_knee_joint 171.2°가 [6.0, 114.0]로 클립된 상태가 기본이라
영구 무장 불가). 고침: 드리프트 기준선을 클립 **전** 원시 실측값(`real_at_sync`)으로
바꿔 실물 대 실물만 비교하고, 클립으로 인한 목표-실물 격차는 새 `clip_warnings()`가 무장을
막지 않으면서 "실물 X° → 적용 Y°, 이동량 Z°"를 deg로 보고한다(`GET /tx/status`와
`POST /tx/arm` 응답의 `sync.clip_warnings`). 같은 세션에서 물리적으로 없는 모터가
HUPHY 관례상 `0.0`으로 sync되는 것도 발견 — 이제 diag(ack/miss/motor_age_ms) 기준 `dead`
판정인 관절은 `"no real data (motor not responding)"`로 스킵해 제외한다. 테스트 4건 추가
(전체 스위트 451개, 무실패). 실기 재검증은 미완료(사용자가 벤치에서 직접 진행).
...
### 11b. 목업 B 반복 개선 루프 (2026-09-04, 코더, `scenario_B.html`+`scenario_common.js` 한정)

사용자가 3안 중 **B**(좌측 Scenario 탭 + 체크리스트)를 선정, "불편한 이유·사항 5개씩 찾고 고치는
루프를 반복"하라는 지시로 진행. A/C·실제 대시보드(`static/dashboard.*`)·`api.py`는 무변경(다른
코더가 대시보드 동시 작업 중). 각 라운드는 목업 페이지 안의 접이식 "개선 이력"에 발견 5개+조치를
남기고 별도 커밋으로 분리. 상세 표는 세션 보고 참조.

- **Round 1** — 반복전환 클릭수(S3→S1 최소 6클릭) / 로봇 "모름" 자기신고가 실측 확인과 동일하게
  초록 ✓로 보이던 정직성 결함 / 목업 초기상태가 재기동 직후(TX 해제·확인 모름이어야 함)를 거꾸로
  가정 / Scenario 탭이 관측 탭과 동일 층위+탭 이탈 시 시나리오 이름 소실 / TX 무장 진입경로가
  체크리스트 모달과 상단바 `confirm()`으로 이원화 — 5건 모두 수정: 축별 "일괄 진행"(TX 무장은 항상
  모달 경유), SSH 전환에 가상 지연+"실패로 표시(모의)" 데모 버튼, 초기상태를 재기동-직후로 정정,
  상단바 상시 시나리오 칩+탭 3그룹 라벨링, TX 무장 경로를 단일 함수로 통합(로봇 미확인 시 동의
  체크박스로 비례 마찰).
- **Round 2** — Round 1이 새로 만든 문제 위주로 재비판: "일괄 진행" 버튼이 클릭마다 다른 일을
  하면서 겉모습은 똑같아 위험한 TX-무장 클릭이 은폐됨 / robotSwitchPending이 전역 슬롯 하나라
  다른 항목에서 전환을 시작하면 먼저 시작한 전환이 로그 없이 조용히 사라짐 / "실패로 표시(모의)"가
  상태를 '불일치'로 만들었는데 실패한 SSH는 그런 근거가 아님(정직성 원칙을 Round 1 자신이 위반) /
  동의 체크박스가 robotConfirm!=='confirmed'로만 켜져 "확인됐지만 다른 프로그램" 케이스를 놓침 /
  안내 모드에서 로봇 확인만 남으면 "일괄 진행"이 진행 없이 반복 로그만 남김 — 5건 수정: 버튼
  라벨/색을 다음 클릭의 실제 동작에 맞춰 재계산(위험 클릭만 다른 색), 다른 축 전환 중엔 새 전환
  시작 거부(로그로 사유 명시), 실패 데모를 '모름'으로 정정, 확인됨+프로그램 불일치도 동의 체크박스
  트리거에 포함, 안내 모드의 진행불가 케이스를 버튼 비활성화로 명시.
- **Round 3** — 라벨-동작 불일치(모드전환 라벨을 단 클릭이 로봇SSH까지 조용히 시작)/개별 "TX 무장"
  버튼이 배치 버튼과 달리 무채색/'불일치' 상태를 시연할 방법이 사라짐(Round 2가 실패데모를 '모름'
  으로 정정한 부작용)/"모름 (프로그램명)" 표기의 거짓 정밀도/상단바 시나리오 칩의 인라인
  text-overflow가 브라우저에 따라 안 먹을 위험 — 5건 수정: 모드전환 후 매번 return(한 클릭=라벨
  그대로), 개별 TX버튼도 위험색 통일, "하트비트가 다른 프로그램을 감지함" 시연 버튼 추가로 '불일치'
  복원, 상태별 문구 재작성(신뢰수준 명시), `.scChip`에 `display:inline-block` 추가.
- **Round 4** — 공유 데이터(`ROBOT_PROGRAMS`)에 S1뿐 아니라 S2 대상(`policy_exec_obs_stream`)도
  `torque:'ON'`으로 기록돼 있는데 로봇 프로그램 전환 컨트롤(버튼/자기신고/일괄진행) 전부가 토크
  OFF 대상과 동일하게 무채색이었음(위험이 한 곳에서만 보인다는 전제가 데이터와 어긋남) / TX 무장
  중에 로봇 프로그램을 SSH로 바꿀 수 있어 명령 수신 중인 서비스를 재시작하는 순서 위험 / Round
  3의 새 "하트비트 불일치 시연" 버튼이 `fixRobot()`을 우회해 Round 2가 만든 "다른 전환 조용히
  취소 금지" 안전장치를 무력화 / 개선이력 블록이 3라운드 만에 30항목으로 불어나 첫 인상을 압도 —
  4건 수정: 로봇 전환이 torque-ON 대상이면 버튼/자기신고/배치라벨 전부 위험색+배너로 통일, TX
  무장 중 자동 로봇전환 거부(안내모드 명령표시는 유지하되 경고 문구 추가)+일괄진행에 "이미 무장된
  상태면 먼저 해제" 단계 선행 추가, 하트비트 시연이 다른 전환을 취소하면 로그에 남김, 이력 블록을
  라운드별 `<details>`로 중첩(최신만 기본 펼침).

**멈춘 지점 (2026-09-04)**: Round 5 재비판에서 발견 1건만 나옴(로봇 전환 버튼이 TX 무장 중일 때
클릭해도 겉보기엔 조용히 거부되고, 로그 줄만 남아 왜 안 됐는지 즉시 안 보임 — 안전하게 실패하지만
조용함) — 5개를 채우지 않고 여기서 루프 종료. **커밋 계보 이상**: round1/2 커밋(`d8f3556`,
`ebe6207`)은 `main`에 있으나, round3+4 커밋(`7306201`)은 두 커밋 사이 어느 시점에 HEAD가
`Viewer-Prototype` 브랜치(코더가 만들지 않음, 동시 작업 중이던 다른 코더 쪽에서 생성된 것으로
추정)로 옮겨간 뒤 커밋되어 그 브랜치에 얹혔다. `Viewer-Prototype`은 현재 `main`보다 정확히 1커밋
앞선 상태(fast-forward 가능)지만, 다른 코더가 그 브랜치에서 작업 중일 가능성이 있어 코더가 임의로
브랜치를 옮기지 않았다 — 병합 여부는 사용자/코디네이터 판단 필요.

## 12. HUPHY `biped` 구조 이행 — API 래퍼·텔레메트리 변환 레이어 재작성 (2026-09-04, 코더)

`~/external_repos/HUPHY` `biped` 브랜치(벤치에도 동일 브랜치 배포됨)에 맞춰 `tools/pygviewer/`
전체(HUPHY 리포 자체는 무수정)를 재작성. 배경: §11 이전까지의 계획은 "우리 포크"에 `build_robot`
디스패치·`robots/single.py`를 HUPHY 소스에 직접 추가하는 방향이었다(docs/123 §5의 "구현 중" 항목,
`build_robot`(kind 분기)/`SingleJoint(Robot)`/HUPHY 전체 995 passed 기록). **`biped` 브랜치는 그
경로를 쓰지 않는다** — 대신 HUPHY 자신이 `robots/biped.py`(`Biped` 클래스, 사지 여럿을 하나의
`Robot` 계약으로 합성)를 새로 갖췄고, `build_robot`/`kind:"single"`은 이 체크아웃에 아예 존재하지
않는다. 즉 이전 계획의 전제(우리가 HUPHY에 단일-모터 지원을 얹어야 한다)가 상위 브랜치 자체의
설계 변경으로 무효화됐다 — 아래는 그 새 구조에 맞춘 재작성이다.

### 확인된 HUPHY biped 구조 (읽기전용 조사)

- 빌더는 `build_leg`/`build_biped` 둘뿐. `Leg`는 여전히 6모터(`hip_pitch/hip_roll/hip_yaw/knee/
  ankle_a/ankle_b`, `robots/leg.py` `REQUIRED_MOTORS`)를 강제한다 — 풀린 것은 **로봇 합성 층**뿐이고
  개별 다리 계약은 그대로다. `build_biped(robot_cfg, limbs=None, **leg_options)`는 `limbs` 생략 시
  `kind: leg`인 사지 전부를 묶어 `Biped`를 만들고, `reader=True`(수신 스레드)가 기본이다.
- **이름 규약**: `Biped`는 모든 관절/모터/텔레메트리/액션 키에 소유 사지의 `Leg.id`를 `/`로
  접두한다(`robots/biped.py`: "관절 이름에 팔다리 이름이 붙음" — `right_leg/knee`). `Leg.id`는
  `robot.yaml`의 `limbs` 키가 그대로 온다. 텔레메트리 와이어 포맷 자체(`{limb}/{motor}/{field}`)는
  바뀌지 않는다 — `limb` 자리에 들어가는 값의 어휘만 `left`/`right`(구)에서 `left_leg`/`right_leg`
  (신)로 바뀐다. `Biped.split_action`은 접두어 없는 이름을 **하드 에러**로 낸다(조용히 버리지
  않음) — 이게 이번 재작성에서 유일하게 코드가 새로 알아야 하는 사실이다.
- 액션은 여전히 **관절 단위**(`Leg.action_features`: hip_pitch/hip_roll/hip_yaw/knee/ankle_pitch/
  ankle_roll — 발목은 pitch/roll로 받고 `Leg` 내부가 a1/a2 IK를 함, 우리 쪽 FK 변환 로직 불변).
- `ControlLoop(robot, hz=..., telemetry=..., mode=Mode.CONTROL|OBSERVE)`는 `Robot` 계약만 쓰므로
  `Biped`가 그대로 들어간다. `Mode.CONTROL` 진입 시 `robot.enable()`(전 사지 all-or-nothing),
  `OBSERVE` 진입 시 `disable()`.

### 재작성 내용

1. **`bridge/joint_map_biped.json`(신규)** — `joint_map_huphy.json`을 대체하는 새 기본 맵. limb을
   `left_leg`/`right_leg`로, 12행+ankle_joints 4행 불변식은 그대로. 벤치에 없는 모터(hip_*, ankle_*)
   행도 남긴다 — "없음"과 "매핑 누락"은 다른 상태이고 후자는 조용한 실패가 된다. 옛
   `joint_map_huphy.json`은 디스크에 그대로 남아 `--map`으로 명시하면 계속 읽힌다
   (`bridge/huphy_udp.py`의 `LEGACY_MAP_PATH`).
2. **`bridge/huphy_udp.py`** — `DEFAULT_MAP_PATH`만 `joint_map_biped.json`으로 교체. 파서
   (`HuphyBridge.parse_fast`/`parse_imu`, `JointMap`) 자체는 이전부터 limb 이름을 하드코딩하지
   않았다(맵 파일에서 그대로 읽음) — 코드 변경 없이 맵 스왑만으로 새/옛 어휘 양쪽을 계속 지원.
3. **`bridge/huphy_remote_motion.py`(핵심 재작성)**:
   - `build_robot`→`build_biped`. `Biped.connect()`/`enable()`이 보유한 전 사지에 대해
     all-or-nothing이라(하나라도 없으면 실패), **이 도구가 커맨드할 딱 한 사지만**
     `limbs=[limb_cfg]`로 넘겨 조립한다(biped 기본값인 "robot.yaml의 kind:leg 전부"를 쓰지 않음) —
     물리적으로 없는 반대쪽 다리 때문에 매 실행이 그 다리의 CAN 버스에서 실패하는 것을 막기 위한
     의도적 선택(§`resolve_side`/`build_biped` 결정, 코드 주석에 명시).
   - `resolve_side()` 재작성: 맵 자신의 어휘(검증 1순위) → 맵 없음(순수 헬퍼 테스트 경로, 구
     `left`/`right`/`left_leg`/`right_leg` 그대로) → 맵은 있으나 못 찾음(역사적 별칭
     `left`/`left_leg`/`leftleg`/`bench` ↔ `right`/`right_leg`/`rightleg` 버킷으로 "이 맵의 그
     사이드가 실제로 뭐라 불리는지"를 재해석) 순. 코드에 특정 limb 이름을 하드코딩하지 않는다.
   - **액션 키 접두어** — `RemoteMotion.__call__`이 만드는 액션 딕셔너리는 여전히 맨몸 관절 이름
     (`hip_pitch`, `ankle_pitch`...)이고, 반환 직전 마지막 한 줄에서만
     `{f"{self.action_prefix}/{k}": v for k, v in action.items()}`로 접두어를 붙인다
     (`huphy_remote_motion.py:699` 부근). `action_prefix`(실제 `Leg.id`/biped 사지 이름)와 `side`
     (조인트맵 자신의 limb 키, 매퍼/데드맨/게인플랜이 쓰는 그룹핑 키)는 개념이 다른 두 값으로
     분리했다 — 보통 같은 문자열이지만, 구 맵(`left`)을 신 biped robot.yaml(`left_leg`)에 대고
     쓰는 경우 실제로 갈린다(테스트로 검증).
   - 안전장치(2단계 무장, 데드맨, `--enable` 허용목록, 게인 상한 클램프, 연결 직후
     `clear_fault()` — 이제 `biped.parts` 전체를 순회) 전부 이식·유지.
   - 진단 출력을 전부 `flush=True`로, 그리고 수신 통계(accepted/rejected_*/parse_errors)를 종료
     시뿐 아니라 `--stats-interval-s`(기본 5초)마다 백그라운드 스레드로 출력하도록 추가 — "TX는
     보내는데 모터가 안 움직임" 류 증상의 원인(토큰 불일치 등)을 실행 중에 바로 보기 위함.
4. **`bridge/tx_map.py`/`tx_client.py`** — 둘 다 애초에 limb 이름을 하드코딩하지 않았음(맵에서
   읽음) 확인, docstring만 갱신. `tx_client.py`는 코드 변경 없음.
5. **`pygviewer/api.py`** — `_read_side_mapping_verified()`가 `joint_map_huphy.json` 파일명을
   하드코딩하던 것을 `bridge.huphy_udp.DEFAULT_MAP_PATH` 참조로 교체(대시보드 UNVERIFIED 배지가
   항상 브리지의 실제 기본 맵을 따라가도록).
6. **테스트** — 새 맵 로딩/12행 불변식/미등록 쌍 거부, 액션 키 접두어 생성(3건, 가짜 `Leg` 객체로
   `RemoteMotion.__call__`을 huphy 없이 직접 구동 — 맨몸 이름이 절대 안 나감을 검증), `--limb`
   별칭 해소+미지 limb 에러 메시지, 파서 신/구 맵 양쪽. 기존 `test_dummy_rx.py`/`test_tx_wiring.py`
   도 `DEFAULT_MAP_PATH`를 통해 간접적으로 새 어휘를 쓰게 되어(`DummyRx`가 맵을 명시하지 않음)
   텔레메트리 키 리터럴(`"left/knee/"`→`"left_leg/knee/"`)을 갱신 — 이 두 파일은 브리핑 대상
   범위 밖이라 처음엔 놓칠 뻔했음(전체 스위트 실행으로 발견). `pygviewer` 전체 스위트 그린
   (`CUDA_VISIBLE_DEVICES="" mujoco-sim/mjlab/.venv/bin/python3 -m pytest`, exit 0).

### 의심스러웠던 점 (그대로 적어둠, 흡수하지 않음)

- docs/123 §5의 "구현 중" 항목(`build_robot`/`robots/single.py`를 HUPHY 소스에 직접 추가, "HUPHY
  전체 995 passed")은 **biped 브랜치와 다른 갈래**의 작업으로 보인다 — 그 갈래가 어느
  브랜치/포크에 실제로 병합됐는지, biped 브랜치가 그 작업 이후에 리베이스된 것인지는 이번 조사
  범위 밖(HUPHY 리포 히스토리 비교는 하지 않음, 지시된 "절대 수정 금지"와 별개로 시간상 생략).
  두 갈래의 관계를 문서만으로는 확정할 수 없다는 점을 여기 남긴다.
- `build_biped(..., limbs=[하나])`로 항상 사지 하나만 조립하기로 한 결정은 이번 작업의 판단이며
  HUPHY 문서가 명시적으로 권장/금지하지 않는다 — 양다리를 동시에 커맨드하려면(장래) 이 도구를
  `--limb` 다중 지정으로 확장하거나 사지별 프로세스를 따로 띄우는 두 갈래 중 하나를 나중에
  정해야 한다.

## 13. 고장 표시 (fault visibility) — "정상으로 보였는데 실제로는 멈춰 있었다" 사건의 후속 (2026-09-05, 코더)

> 배경: 벤치에서 모터 2대가 과전압 고장으로 동시에 멈췄는데, 화면은 170초간 계속 "정상"으로
> 보였다(통신 110 Hz·ack=1·miss=0, 무릎 각도만 141.40도로 얼어붙음, 온도는 3308.8도라는 깨진
> 값 — 고장을 지우니 34.0도로 정상 복귀). 근거는 `docs/124_huphy_handoff_findings.md` 1·2절.
> HUPHY 리포는 이번에도 건드리지 않았다(read: `tools/pygviewer/` 안에서만 작업).

### 13a. 명령 무시 감지 + 고장코드 읽기 + 화면 표시 (완료, 커밋 아래)

두 층으로 나눴다 — 제어 주기 안에서 고장을 조회하면 상태 프레임과 CAN 번호가 같아 오인될
위험이 있다는 지적(docs/124 §2)을 그대로 반영한 구조다.

1. **(a) 얼어붙음 감지 — 추가 통신 없음, 항상 동작.** 새 순수 모듈
   `tools/pygviewer/pygviewer/bridge/motor_fault.py`의 `StuckDetector`가
   `RemoteMotion.__call__`에서 매 틱(무장 여부·phase 무관) 판정한다. 조건 3가지가 동시에
   `STUCK_HOLD_S`(1.0초) 이어지면 신고:
   - `STUCK_ERR_DEG` = 3.0 deg — 목표·실측 차이가 이보다 커야 함
   - `STUCK_POS_DEADBAND_DEG` = 0.2 deg — 이 구간 동안 실측 위치가 이보다 더 움직이면 리셋
   - `STUCK_TAU_ZERO_NM` = 0.05 N·m — 보고된 토크가 이보다 크면(=실제 부하와 씨름 중) 리셋
   - hip_pitch/hip_roll/hip_yaw/knee 4개 단일 모터만 대상 — 발목 pitch/roll은 FK로 유도된
     값이라 원 크랭크 모터의 토크 신호가 이 관측 공간에 없음(§13c "미해결" 참조).
2. **(b) 고장 코드 읽기 — 버스가 조용할 때만.** 새 클래스 `FaultPoller`가 "무장 해제 순간
   즉시 1회 + 이후 `FAULT_POLL_INTERVAL_S`(1.0초)마다"만 조회를 허용하고 무장 중엔 항상
   거부한다. 실제 조회는 `run_real()`의 `_make_can_fault_query_fn`이 이 leg의 CAN 채널에
   **독립적인 `python-can` 핸들**을 하나 더 열어(SocketCAN은 한 인터페이스에 여러 리스너를
   허용 — `candump`가 실트래픽과 공존하는 것과 같은 원리) `deploy/bench/read_fault_raw.py`와
   동일한 프로토콜(정체 프레임 비우기 → 고장조회 프레임 전송 → 응답 1개 수집)로 원시 8바이트를
   받는다. `bus.read_fault()`의 자체 해석은 쓰지 않는다 — docs/124 §1이 지적한 바이트 순서
   버그가 이 로봇의 HUPHY 체크아웃에 아직 남아있는지 알 방법이 없기 때문. 대신
   `decode_fault_word()`가 같은 4바이트를 **little(정답, 제조사 SDK/매뉴얼 기준)과
   big(HUPHY 현재 방식) 두 가지로 항상 함께** 해석해 절대 하나만 보여주지 않는다.
3. **화면 표시.** 로봇 쪽(`RemoteMotion`)이 별도의 작은 UDP 텔레메트리 소켓으로
   `{limb}/{motor}/stuck`·`fault_le`·`fault_be` 3개 숫자 필드를 HUPHY 자체 텔레메트리와
   **같은 목적지에 별도 데이터그램으로** 보낸다(HUPHY의 `Telemetry` 클래스는 절대 수정 금지라
   확장 지점이 없으므로) — 뷰어의 `HuphyBridge`가 이미 하던 대로 각 데이터그램을 독립적으로
   파싱하므로 HUPHY 자체 스트림과 충돌 없이 공존한다. `schema.JointState`에 세 필드 추가,
   `telemetry.RealState`가 저장하고, 관절 헬스 그리드에 **`fault_reason`**을 새로 얹었다 —
   기존 ok/warn/dead(통신 상태)와는 **완전히 독립**이라서, 이번 사건처럼 통신은 멀쩡해 보이는데
   관절이 실제로는 명령을 안 따르는 경우도 절대 "정상"으로 보이지 않는다(대시보드
   `.health-fault` 빨간 셀 + 툴팁, `GET /violations`의 `side="stuck"`/`"fault"` 기록에 쉬운 말
   한 줄 — 예: `"L_knee_joint: 명령을 따르지 않음 (고장 의심) — 목표 114.0 (deg), 실측 141.4
   (deg), 토크 0.00 N·m, 12초째"`, `"L_knee_joint: 모터가 과전압(으)로 힘을 끊었습니다 (코드
   0x00000008)"`).

### 13b. 확인되지 않은 가정 (그대로 적어둠, 흡수하지 않음)

이 세션은 모터가 꺼진 상태에서 작업했고 하드웨어 검증은 지시상 사용자 몫이다. 실물 확인이
필요한 지점 3개:

- `observation`에서 토크를 읽는 키 이름(`OBS_TAU_SUFFIX = ".tau"`)은 이미 확인된
  `".pos"`(docs/123)와 같은 규칙을 따를 것이라는 **추정**이다. 틀렸다면 `StuckDetector`는
  토크를 항상 "없음"으로 보고 **얼어붙음을 절대 신고하지 않는 쪽으로만** 안전하게 실패한다
  (오탐은 없고, 미탐 가능성만 있음 — `huphy_remote_motion.py`의 `OBS_TAU_SUFFIX` 주석 참조).
- `run_real()`의 `_make_can_fault_query_fn`이 CAN 채널을 여는 데 쓰는 `limb_cfg.channel`/
  `limb_cfg.interface`, 그리고 모터 id↔이름 매칭에 쓰는 객체-identity 역조회는 HUPHY 내부
  구조에 대한 최선의 추정이다 — 틀리면 시작 시 `fault code reading unavailable` 경고가 뜨고
  (a)층(얼어붙음 감지)만 동작한다, 컨트롤 루프가 죽지는 않는다. 벤치에서 이 경고가 안 뜨는지
  확인 필요.
- 발목(ankle_pitch/roll) 관절은 얼어붙음 감지·고장코드 표시 대상에서 **의도적으로 제외**했다
  (위 13a-1 참조) — 필요하면 별도 설계가 필요하다.

### 13c. 과열 컷오프 (완료 — 구현·단위 시험까지, 하드웨어 검증은 사용자 지시로 보류)

사용자 지시: 50도 이상이면 그 관절 세기 0으로 컷오프, 45도 이하로 식으면 자동 재개. 온도가
−20~150도 범위 밖(이번 사건의 3308.8도 포함)이면 컷오프하지 않되 "온도를 읽을 수 없음"을
계속 표시. **온도 결론 정정(사용자, 2026-09-05)**: 3308.8도는 온도 인코딩 문제가 아니라
**고장 상태에서 읽힌 깨진 값**이었다(고장을 지우니 34.0도로 정상 복귀) — 13a의 고장 표시와
같은 근본 원인으로 다룬다. **사용자 지시로 하드웨어 검증(모터를 실제로 데워 컷오프가 걸리는지
보는 것)은 이번엔 하지 않음** — 구현 + 단위 시험까지만.

- `bridge/motor_fault.py`의 `ThermalCutoff`(13a와 같은 파일, 순수 클래스)를
  `RemoteMotion.__call__`에 배선: 매 틱(무장 여부·phase 무관, 13a와 같은 이유 — 통신은
  멀쩡해 보이는데 관절이 과열 중일 수 있음) `_update_thermal_cutoff`가 판정하고,
  실제 명령 경로(무장/hold/returning/default 단계)에서만 `_cut_motor_names`를 반영해 그
  관절을 세기 0·목표=마지막 관측 위치로 덮어쓴다 — `_idle_refresh_action`이 이미 쓰던
  "합성 목표로 몰지 않는다" 원칙 그대로. **적용 순서가 중요**: 게인 계획(`plan_gains`)이
  먼저 그 관절에 진짜 게인을 매길 수 있으므로, 컷오프 덮어쓰기는 반드시 그 다음에 온다 —
  아니면 컷오프가 곧바로 지워진다(시험으로 확인).
- 온도를 읽는 관측 키(`OBS_TEMP_SUFFIX = ".temp"`)도 13b와 같은 **추정**이며 같은 방향으로
  안전하게 실패한다(키가 틀리면 컷오프가 절대 안 걸리는 쪽으로만 실패, 잘못 걸리는 쪽은 없음).
- 화면 반영: `stuck`/`fault_le`/`fault_be`와 같은 보조 UDP 텔레메트리에 `temp_valid`·`cutoff`
  두 필드 추가(`schema.JointState`, `huphy_udp.py`), `telemetry.RealState`의 `fault_reason`에
  "과열로 힘을 끊음"·"온도를 읽을 수 없음" 문구를 더했다 — 13a의 `fault_reason`과 **같은
  칸**에 합쳐 보인다(관절 하나가 얼어붙음+과열을 동시에 겪어도 두 줄로 안 갈라짐).
  `GET /violations`엔 `side="cutoff"`(컷오프/재개 전환 시점에만 기록, 매 틱 아님)와
  `side="temp_unreadable"` 두 종류가 새로 쌓인다.
- 새 시험 11건(총 504건, 기존 493건에서 +11): `RemoteMotion`이 51도에서 세기 0으로 바뀌고
  44도에서 재개하는지, 3308.8도(이번 사건 실측값)를 주면 컷오프하지 않고 "읽을 수 없음"만
  뜨는지, 관절 하나만 과열됐을 때 다른 관절은 안 건드리는지, 컷오프/온도-무효 텔레메트리가
  실제로 나가는지(`test_remote_motion.py` +5), 수신측 스키마·브리지·`fault_reason`·위반기록
  (`test_fault_visibility.py` +6). `ThermalCutoff` 자체의 순수 로직 시험(51/44/3308.8도 경계,
...
## 15. 지금 무엇을 하는 중인가 (상황 조합) — 2026-09-06

목업 B안(`mockups/scenario_B.html`, 왼쪽 탭 + 체크리스트)을 실제 상태에 연결한 것입니다.

### 원칙

**조합은 화면이 소유한 상태가 아니라, 이미 있는 상태에서 계산한 이름입니다.** 세 축이 정해진
조합과 **정확히** 맞을 때만 이름이 붙고, 하나라도 어긋나면 이름을 떼고 무엇이 다른지 지목합니다.
근사치에 이름을 붙이지 않는 이유는 하나입니다 — 조작자가 그 이름을 믿기 때문입니다.

| 축 | 화면이 아는 방법 | 바꾸는 방법 |
|---|---|---|
| 화면 모드 | `SimCore.mode` | 버튼이 바꿔 줌 |
| 전송 무장 | `TxState.armed` | **버튼이 안 바꿈** — 사람이 sync → ARM → 스페이스 |
| 로봇 프로그램 | **로봇이 스스로 보고** | 사람이 로봇에서 실행 |

### 목업과 달라진 한 가지

목업은 세 번째 축을 SSH로 바꾸고 **성공했다고 믿는** 구조였고, 그 한계를 화면에 적어 두었습니다.
실제 구현에서는 로봇 프로그램이 자기 번호를 **진단 패킷마다** 실어 보냅니다
(`scenario.PROGRAM_*`, 스키마 `JointState.prog`). 그래서 `confirmed` 는 **나이가 붙은 측정값**이고,
로봇이 2초(`PROGRAM_STALE_S`) 넘게 조용하면 스스로 `unknown` 으로 떨어집니다. 죽은 연결에서
이름이 남아 있는 일이 없습니다.

### 왜 버튼이 무장까지 하지 않는가

세 조합 중 실물에 힘이 들어가는 것은 하나뿐입니다. 그 하나에 도달하는 길은 **바뀌지 않았습니다** —
실물 값 맞추기 → 무장 → 스페이스 유지. 버튼 한 번으로 토크가 켜지면, 그 절차 전체가 무의미해집니다.
버튼은 화면 모드만 맞추고 **남은 것을 글로 알려 줍니다.**

### 지금 쓸 수 있는 것

`실물 동시 구동` 하나뿐입니다. `손으로 미러링`·`로봇 정책 관측`은 필요한 로봇 프로그램이 아직
없어서, 화면에 **왜 못 쓰는지 적고 버튼을 비활성**으로 둡니다.

### 곁가지로 고친 것

왼쪽 탭은 각자 "내가 이 몸통을 그렸다"를 표시해야 합니다(`body.dataset.builtTab`). 표시를 안 하면
다음에 원래 탭으로 돌아갈 때 **이미 그려졌다고 착각하고** 사라진 요소를 만집니다. 새 탭이 이걸
빠뜨려 실제로 터졌고(`renderLeftTab: el(...) is null`, 2026-09-06 04:04), 고치면서 보니
`Model`·`Status` 탭도 원래부터 표시를 안 하고 있었습니다. 셋 다 고치고 시험으로 묶었습니다
