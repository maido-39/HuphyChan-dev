# 106. 세션 지시사항 종합 + 계획 (2026-08-27 재정리 / 2026-09-03 공백기 반영)

> ★2026-09-03 갱신: 세션 공백기(08-31~09-03) 작업은 [[118_session_gap_recovery]] 참조.
> 그로 인한 본 문서 상태변경 — **B1 v2s1 완주**(측정값은 v30 모델 세대 교체로 구세대 강등),
> **D1 어깨 메쉬 해소**(v30 89/89 재추출), **H2 v2u1 재정의**(주라인=LegOnly 12-DOF, 109 게이트).
> 현재 본런 = `legonly_ab_v1`(P2 진행 중). 신규 과제(09-03 사용자): ①보행품질(무릎/AB/toe-off)
> ②collision 메시 검토 ③설계 vs 실측 질량 감사.
...
- ~~(09-03) v30 리셋 키프레임 발바닥 매몰~~ → **해결(09-05)**: 키프레임 그 자체(스텝 없이)에서 실측 **−31.0 mm**(백로그의 38.6은 근사). 원인은 `KNEES_BENT_KEYFRAME`의 `standing_z − 0.04` 규칙이 v30 웅크림 기하와 무관한 상수였던 것. 수정: 모델 태그별 bent 파일에 `base_z`(발바닥 최저점이 바닥 +5 mm가 되는 높이, 전체 로봇 순수 기구학)를 넣고 `_bent_base_z()`가 읽음 → 실측 **base z 0.904 m, 발바닥 +5.0 mm**. 도구 `tools/robot_model/loop_bent_keyframe.py`. 옛 값은 `PYG_BENT_KEYFRAME_LEGACY=1`로 재현 가능(옛 체크포인트 측정용).
- ~~(09-04) 리셋 폐루프 찢김~~ → **해결(09-05)**: 키프레임에서 실측 **37.23 mm**. 원인은 v3 모델에서 푼 크랭크 각(양쪽 −17.12°)을 v30(크랭크 축이 좌우 미러: L_A −Y/L_B +Y/R_A +Y/R_B −Y)에 그대로 복사한 것. 수정: v30 모델 위에서 정강이를 고정하고 크랭크 서보로 발목 +0.36 rad/0 을 뉴턴 반복으로 맞춰 재표현(`LegOnly_..._v30_proxyfix_loop_bent.json`, L_A **+17.03°**, L_B −17.26°, R_A −17.03°, R_B **+17.26°**) → 실측 **찢김 0.00 mm**. `_bent_json_path()`가 모델 태그 파일을 우선. ★주의: 크랭크 기본값이 바뀌므로 **09-05 이전 체크포인트는 `PYG_BENT_KEYFRAME_LEGACY=1`로만 측정**해야 함(정책이 옛 기본값 기준으로 관측·행동을 학습함). 다음 학습부터 적용.
- (09-03, 동상) **RP 변형 ankle_pitch 액션 창 한쪽 5.4°**(default +0.360 vs clip 상한 +0.454) — v1 무릎 0° 창과 같은 계열(경미). RP 런 재개 전 default/clip 재검토.
- ~~(09-04 03:35) mjlab 워크트리 미커밋 실변경 23파일~~ → **해결(09-05)**: 사용자 결정으로 "legonly_ab_v2가 학습한 코드 상태" 스냅샷으로 **일괄 커밋**(mjlab `d0c85448`, 22파일 +3,187/−1,823; 23번째 `pygmalion_constants.py`는 키프레임 수정과 함께 `568a4372`). 내용 심사는 하지 않았고 런의 `repro/`(env_cfgs·curriculums·rl_cfg·xml·mass DR·런처)와 바이트 단위 대조 가능. 미추적 618개는 그대로 둠.
...
| **H2** | 상체 자유도 학습 `v2u1` (v2s1 단일변인 A/B) | ⏸ 발사대기: ✅H1·✅설정확정(dry-run) → **하중 측정 완료만 남음**(측정이 gpu.lock 보유+평가기 RAM스파이크, 겹치면 스왑). 완료 즉시 발사+모니터 |
