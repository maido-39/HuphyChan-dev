# 학습 리포트 — 2026-07-03_04-03-01 (mjlab B1)

- **task/run**: `2026-07-03_04-03-01` (mjlab MuJoCo-Warp + rsl_rl PPO)  ·  **wandb**: `peezxrqs`
- **의도/변경점**: ★ contact_force_cap(−0.005·600N·clip400) — 사뿐착지. A1b 동결 위 단일변수.
...
- **config 백업**: `logs/rsl_rl/pygmalion_velocity/2026-07-03_04-03-01/params/{env.yaml, agent.yaml}` (mjlab은 params/에 저장)
- **체크포인트**: `logs/rsl_rl/pygmalion_velocity/2026-07-03_04-03-01/model_3000.pt` (외 model_*.pt)

## 1b. B1 Reward & Gains (config에서 파싱 — 재현용)
...
| contact_force_cap | -0.005 | ★충격 cap: 발 GRF 역치초과분 벌점(사뿐착지) | -min(max(F-600,0),800) |
...
**§1b-2. 액추에이터 동역학·한계** (이 런의 `params/env.yaml` 파싱 — `2026-07-03_04-03-01`)
...
> ⚠ **파일 최신성 주의** — 이 XML은 런 시작(2026-07-03_04-03-01)보다 뒤인 2026-07-06 22:38에 수정되었다. 런 디렉토리에 모델 스냅샷이 없으므로 아래 range는 **현재 파일 기준**이며 런 당시와 다를 수 있다.
...
## 2. 지표 (Metrics)
- **최종 Mean reward**: 74.64 (iter 3385), max 76.22
- **error_vel_xy**: 0.767 · **error_vel_yaw**: 0.661
- **낙상률(fell_over/low_base 최종)**: 0.000 / 0.167

![[2026-07-03_04-03-01_reward.png]]

## 2b. Reward 기여 (이름 · 값 · 기여 · 무엇 · 왜)
| Reward | 가중치 | 기여(final) | 무엇/왜 |
|---|--:|--:|---|
| `track_angular_velocity` | +2 | +1.4785 | 명령 회전속도 추종 |
| `track_linear_velocity` | +2 | +1.3151 | 명령 전진/측방 속도 추종 |
| `upright` | +1 | +0.9760 | 몸통 직립 유지(넘어짐 방지) |
| `pose` | +1 | +0.6830 | 기본 관절자세 정규화(기괴자세 억제) |
| `action_rate_l2` | -0.1 | -0.3867 | 액션 급변 벌점 |
| `foot_clearance` | -2 | -0.1361 | 스윙발 지면 이격(발끌림 방지) |
| `angular_momentum` | -0.02 | -0.1021 | 전신 각운동량 벌점(회전 낭비 억제) |
| `contact_force_cap` | -0.005 | -0.0917 | ★충격 cap: 발 GRF 역치초과분 벌점(사뿐착지) |
| `foot_slip` | -0.1 | -0.0237 | 접지발 미끄러짐 벌점 |
| `foot_swing_height` | -0.25 | -0.0217 | 스윙발 높이 성형 |
| `self_collisions` | -1 | -0.0042 | 자기충돌 벌점 |
| `dof_pos_limits` | -1 | -0.0042 | 관절범위 한계 벌점 |
...
![[2026-07-03_04-03-01_tensorboard.png]]

- reward -1.3→**74.6**(수렴) · ep_len 최종 990 · 추종 vx 0.767/yaw 0.661 · 낙상 fell 0.00/low_base 0.17

## 3. 학습 진행 영상 (ACCUMULATION)
![[accum_B1.mp4]]
*(iter 캡션付. 원본: `logs/rsl_rl/pygmalion_velocity/2026-07-03_04-03-01/videos/accumulated_progress.mp4`)*

## 5. 분석
GRF P99 −19%(경계)·peak 악화(clip400 gradient 차단). knee 122→104. → B1w2 에스컬레이션.
...
![[2026-07-03_04-03-01_torque.png]]

**관절 속도 RMS/p95/MAX(rpm) vs 속도한계**
![[2026-07-03_04-03-01_speed.png]]

**관절 토크 시계열(peak/rated 선)**
![[2026-07-03_04-03-01_torque_ts.png]]

- 정량: knee RMS 104%·ankle_pitch RMS 115%·binding(RMS>rated): hip_pitch, knee, ankle_pitch
