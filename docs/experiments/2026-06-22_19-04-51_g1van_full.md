...
## 1b. g1van_full Reward & Gains (config에서 파싱 — 재현용)
...
**§1b-2. 액추에이터 동역학·한계** (이 런의 `params/env.yaml` 파싱 — `2026-06-22_19-04-51_g1van_full`)
...
![[2026-06-22_19-04-51_g1van_full_reward.png]]
...
|---|--:|--:|---|---|
| `track_ang_vel_z_exp` | +2 | +1.7986 | 명령 각속도(yaw) 추종 exp | 작업 목표: 방향 전환 추종 |
| `track_lin_vel_xy_exp` | +1 | +0.9520 | 명령 선속도(x,y) 추종 exp | 작업 목표: 원하는 속도로 보행 |
| `joint_deviation_hip` | -0.1 | -0.0474 | hip 중립 이탈 penalty | hip 자세 안정(과회전 억제) |
| `action_rate_l2` | -0.005 | -0.0305 | action 변화율 penalty | 급격한 명령 변화 억제 = smooth |
| `ang_vel_xy_l2` | -0.05 | -0.0107 | roll/pitch 각속도 penalty | 몸통 흔들림 억제 |
| `feet_slide` | -0.1 | -0.0106 | 접지 발 미끄러짐 penalty | 발 고정(slip 방지) |
| `dof_acc_l2` | -1.25e-07 | -0.0067 | 관절 가속도 L2 penalty | 고주파 진동(떨림) 억제 = smooth |
| `feet_air_time` | +0.25 | +0.0037 | 발 공중(또는 single-stance) 시간 보상 | 보폭/스텝 유도(threshold 미달 시 dead) |
| `flat_orientation_l2` | -1 | -0.0025 | 몸통 수평(중력 proj) penalty | 몸통 똑바로 유지 |
...
![[2026-06-22_19-04-51_g1van_full_tensorboard.png]]
...
![[2026-06-22_19-04-51_g1van_full_torque.png]]

**관절 속도 RMS/p95/MAX(rpm) vs 속도한계 가로선 + 포화%**
![[2026-06-22_19-04-51_g1van_full_speed.png]]

**관절 토크 시계열 (시간에 따른 토크 활용, peak/rated 선)**
![[2026-06-22_19-04-51_g1van_full_torque_ts.png]]

**관절 속도 시계열 (시간에 따른 속도 활용, limit 선)**
![[2026-06-22_19-04-51_g1van_full_speed_ts.png]]
