...
| **legonly_ab_v2** [[2026-09-03_legonly_ab_v2]] | v1과 **유일 변인 = 설정 side-aware 수정**(default/clip 부호를 관절 range에서 자동유도: L_knee 창 0°→108°, R_hip_pitch 43°→130.5°; bent 키프레임 폐루프 각 재표현 closure 37.27→0.001 mm; 프리플라이트 게이트 신설). 모델 XML·리워드·레시피·DR 전부 v1 동일 | (학습중) 09-03 13:21 P1 발사 16384 env, 프리플라이트 PASS 12/12 좌우대칭. **model_700 vel-0 정지자세**: base 0.903 m 완전정지, knee 굴곡 L 5.81/R 5.88°(대칭 — 창 열림 실동작 확인), hip_yaw −20/−26° 비틀림(감시), 키프레임 발바닥 10.6° 기울기 발견(백로그) | 🔄 완주 게이트 = 운동학 재측정(무릎 swing 피크 vs 인간 55~65°) → 미달 시 swing knee 항 A/B |
