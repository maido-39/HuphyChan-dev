...
| ★**bundleP1_AB** "low-speed deadband fix (2026-08-27)" (`19-39-45`, 16384 env, AB c3 `model_31999` +800 → 32798, ✅완주 40분) [[2026-08-27_bundleP1_AB]] | D1 레시피 + **`PYG_LOWSPEED_DEADBAND=0.05`** = `stand_still_penalty.cmd_deadband` **0.3 → 0.05**(유일 변인, 플래그 OFF는 컴파일 diff로 바이트 동일 실증). 근거 [[2026-08-27_low_speed_command_research]] §5 P1 — 0.05–0.3 m/s는 정지벌점(0.3)·에어타임(0.5) 두 게이트가 **동시에 꺼지는 사각지대** | 평가기 32 ep × 20 s(교란 전부 ON): **cmd 0.25 달성률 0.388 → 0.885(+128 %)**, 스톨률 0.454 → **0.189(−58 %)**, 오차 0.268 → 0.225. **정지(cmd 0) 보존**: 표류 0.191 → **0.189 m/s**, 체공 0.000 — 영구 스텝핑 회귀 없음. 0.5·1.6 무변화(1.6 오차 +0.003), 낙상 0, 생존 32/32. 학습 reward 87.24 → 82.22(중앙값; −5.02 = 새 벌점 자체), `stand_still_penalty` −0.109 → **−0.198**(첫 100 iter, 100/100 nonzero = term-alive 통과). ★**200 Hz 프로브(24 env, 1.6): 하중률 D1 20.6 → P1 46–58 BW/s = 2.2–2.8배**(양 arm 2회씩 반복, D1은 오늘 코드로 재측정해 아카이브 20.98과 일치 확인). **피크 GRF 1.22 BW·착지 2.2/s는 무변화** — 힘·빈도가 아니라 **상승률만** 악화(p90 64 → 105) | ⚠ **조건부 — 플래너 판단 대기**. 저속 문제는 해결됐고 메커니즘도 확정(cmd 1.6에선 양 arm 보상이 동일한데 착지가 바뀜 = 저속 대역 그래디언트가 공유 신경망의 전 속도대 착지를 끌고 감). 그러나 하중률 2.2–2.8배는 [[2026-08-27_bundleE1_AB]]를 **기각시킨 바로 그 지표**(2.6배)라 무시하면 일관성이 깨진다. 단 P1은 형제 모집단에선 중간값(C3 41.4·V4 42.2 < P1 < E1 54.1·C5 69.7)이고 **D1(21.0)·TRT(23.7)·B2(28.6)가 예외적 저값 그룹** — "P1이 나쁜" 게 아니라 "D1이 잡고 있던 최저 하중률을 잃은" 쪽. 선택지 A(그대로 채택·사이징 재산출) / **B(시드 2–3 arm으로 deadband 필연 vs 재학습 변동 분리, arm당 40분)** / C(착지항 상쇄, ★50 Hz `foot_loading_rate`는 E1 교훈상 금지) — **추천 B→C**. ★**P2(`air_time` 0.5 → 0.05) 에스컬레이션 불필요**(P1 단독으로 저속 해결). ⚠ 측면·후진·회전 저속 미측정 |
