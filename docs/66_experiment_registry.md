...
| ★**bundleP1_AB** "low-speed deadband fix (2026-08-27)" (`19-39-45`, 16384 env, AB c3 `model_31999` +800 → 32798, ✅완주 40분) [[2026-08-27_bundleP1_AB]] | D1 레시피 + **`PYG_LOWSPEED_DEADBAND=0.05`** = `stand_still_penalty.cmd_deadband` **0.3 → 0.05**(유일 변인, 플래그 OFF는 컴파일 diff로 바이트 동일 실증). 근거 [[2026-08-27_low_speed_command_research]] §5 P1 — 0.05–0.3 m/s는 정지벌점(0.3)·에어타임(0.5) 두 게이트가 **동시에 꺼지는 사각지대**라 걷기 비용만 물고 보상이 없었다 | 평가기 32 ep × 20 s(교란 전부 ON, 시나리오당 별도 호출·8 병렬): **cmd 0.25 달성률 0.388 → 0.885(+128 %)**, 스톨률 0.454 → **0.189(−58 %)**, 오차 0.268 → 0.225. **정지(cmd 0) 보존**: 표류 0.191 → **0.189 m/s**, 체공 0.000 — 영구 스텝핑 회귀 없음. 0.5·1.6 무변화(1.6 오차 0.278 → 0.281, **+0.003**), 낙상 0, 생존 32/32 전 시나리오. 학습: reward 87.24 → 82.22(중앙값; **−5.02는 새 벌점 자체**, 행동지표 무변화), `stand_still_penalty` −0.109 → **−0.198**(첫 100 iter 창, 100/100 nonzero = term-alive 통과) | ✅ **채택** — 확정 레시피에 플래그 추가. ★**P2(`air_time` 임계 0.5 → 0.05) 에스컬레이션 불필요**: P1 단독으로 해결됐으므로 [[2026-07-05_periodic_contact_removal]]이 의도적으로 잡은 정지친화 값을 되돌리는 헛디딤-게이밍 위험을 살 이유가 없다. ⚠ 측면·후진·회전 저속 **미측정**(이번 격자는 전진 4점) — V2 본학습 전 fc 121 격자로 확인. ⚠ 절대 오차값은 D1 노트(0.133)와 **교란 설정이 달라** 직접 비교 금지(양 arm 동일 조건 재측정이라 A/B는 유효). ★청크 함정 실증: 1.6 오차를 8 ep만 보면 +0.057(뒤집힘), 32 ep 전량은 +0.003 |
