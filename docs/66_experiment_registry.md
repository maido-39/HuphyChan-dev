...
> 동기화 확인 2026-08-28: ankleAB/RP_c3 노트의 최신화는 review_loop 자동 스냅샷 행(신규 실험 아님) — 행 변경 없음.
> 리서치 노트 2026-09-05: [[2026-09-05_legonly_v2_gait_defects_root_cause]] — 좌우 비대칭(시작 자세 크랭크 기본값 부호 오류, 데이터 확인)·저속 정체(임계 사각지대 0.3~0.5, 학습 명령의 1.15 %)·짧은 지지(체공 보상 ON·지지 항 없음·높이 기준 OFF). 제안 A0~A4는 노트 §7.
> 동기화 확인 2026-09-05 13:40: legonly_ab_v2 완주 측정 완료 — 무릎 스윙 63~70°(좌) 확인, 전체 명령 범위 외란 없음·주입 측정, §7 모터 활용도 기록. 다음 학습 계획은 노트 §2c-2.
> 분석노트(런 아님) 2026-08-28: [[2026-08-28_ab_ankle_usage_audit]] — AB 발목 미사용 의혹 감사, 버그 없음(발목 full ROM·tauank_eq 20N·m·전달 0.999). 학습런 없음, era표 행 불필요.
...
| **v2s1_AB** "V2 main, from scratch, 2-phase (2026-08-27)" (16384 env, P1 게이트졸업 ~4100 → P2 DR램프 ~18.1k, 🔄) [[2026-08-28_v2s1_AB]] | **처음부터** 확정 전체스택(착지레시피+v4 31.3kg+vy스테이지+게이트커리큘럼+criticDR 82ch+P2 entropy어닐링), **deadband 제외** | P1 stage0→1@800 **gate-passed(강제아님)**·낙상0·err 0.183. P2 DR만배 ~15.4k/18.1k. 완주후 fc/fcp+200Hz+§7 측정 | 🔄 진행 — **본학습** |
...
| 런 | 변인 (vs 직전) | 정량 | 판정 |
|---|---|---|---|
| **legonly_ab_v1** [[2026-09-03_legonly_ab_v1]] | v2s1 풀스택 레시피 그대로, 16384 env 본학습. 모델만 LegOnly로 교체(23.630 kg, 12-DOF), DR은 fastener50 합본 leg-only 서브셋(스모크가 쓴 구버전 대체) | P1 게이트 졸업 ~4500 iter(낙상 0.000), P2 전이 dip 1h 내 회복(69→91.6), iter 5.6k reward 92.3·ep_len 994·DR 램프 0.11·vx_max 2.5 — 게이트 판정 "계속"(노트 §2c) | ❌ **09-03 12:05 보수적 중단(iter ~5.7k) — 런 무효**: v30 미러축 vs 단일 default/clip → L_knee 액션창 0°(운동학 노트로 확정). 후속=설정 side-aware 수정+`legonly_ab_v2`. (중단 전 기록: P2 5.6k/18.5k. wandb entity `dongyub39-snu`는 **정확**(v2 발사 시 wandb가 동일 슬러그로 sync 확인) — 사용자가 본 404는 비로그인/비공개 프로젝트 접근으로 추정. ⚠질적 플래그: stiff-knee/AB 미활용/toe-off 부재(사용자 09-02 관찰) → 운동학 정량화+리워드 연구 진행 중 |
| **legonly_ab_v2** [[2026-09-03_legonly_ab_v2]] | v1과 **유일 변인 = 설정 side-aware 수정**(default/clip 부호를 관절 range에서 자동유도: L_knee 창 0°→108°, R_hip_pitch 43°→130.5°; ~~bent 키프레임 폐루프 각 재표현 closure 37.27→0.001 mm~~(09-04 정정: 미적용, reset closure 37.27 mm 실측); 프리플라이트 게이트 신설). 모델 XML·리워드·레시피·DR 전부 v1 동일 | (학습중) 09-03 13:21 P1 발사 → 19:48 P1 졸업(4100) → P2 진행 09-04 15:15 iter ~14.7k/18099·DR 1.0 도달·reward ~108·낙상 0(v1 동일 단계 err 1.2). **게이트1(14:32, iter 685)**: reward 110.5 vs v1 동일 iter 64.5, err_xy 0.461 vs 1.123, σ 0.183 vs 0.317, 낙상 0 — 전 지표 우세, 무릎 창 픽스 효과로 해석 → 판정 **계속**. **model_700 vel-0 정지자세**: base 0.903 m 완전정지, knee 굴곡 L 5.81/R 5.88°(대칭 — 창 열림 실동작 확인), hip_yaw −20/−26° 비틀림(감시), 키프레임 발바닥 10.6° 기울기 발견(백로그). **자동 스냅샷 추이**(vx_max 커리큘럼 0.8→2.0 m/s 램프 중 자연 변화): 15:26 iter 1321 reward 111.4·err_xy 0.586 → 16:26 iter 1957 reward 107.8·err_xy 0.751 → 17:26 iter 2593 reward 102.9·err_xy 0.977 → 18:26 iter 3228 reward 102.1·err_xy 1.017·σ 0.243, 낙상 전 구간 0.000~0.125, 속도 ~11 iter/min. **게이트 2~5 일괄 판정(18:36)**: vx_max 0.8→2.5 램프 완료에 따른 reward/err 하락은 명령 난도 상승의 정상 추세(v1 동일 단계 3765: err_xy 1.327·σ 0.315 대비 우세), 낙상 0 유지, thermal 2.17 → **계속**. 최종 stage dwell 106 iter → P1 졸업 ETA 19:40~20:30. **게이트 6(19:38, iter 3867)**: reward 99.3·err_xy 0.957(개선 추세)·낙상 0, dwell 754/800, 런처 게이트 err 비 1.06(<1.1) → 졸업 근접, 안정 스트릭 채우면 P2 자동전이 → **계속**. ⚠watch: thermal 2.49(v1 동일 단계 2.22)·ep_len 986(low_base 0.167), 중단 사유 아님 | 🔄 진행 중 — **P1 졸업(19:48, iter 4100)** 게이트 커리큘럼(dwell ≥800+안정 스트릭, gate err 0.247 vs base 0.234) 통과 → 런처가 model_4100 FULL RESUME으로 **P2 발사(19:49, 18,100 iter, 질량 DR+push 램프 window 4100→14100)**, 와치독·리뷰루프 p2 재등록. 노트 §1b에 리워드 가중치·관절별 Kp/Kd/effort/T-N·ROM/clip 표 추가(20:05). **P2 첫 스냅샷(21:08, iter 4758)**: reward 102.5(P1 종료 102.1 유지 = 전이 dip 없음, v1은 1 h 회복 필요했음)·err_xy 0.928·낙상 0·ep_len 1000·DR 0.07·thermal 2.65(감시). **게이트 7(21:20)·8(22:27)**: DR 램프 0.07→0.12에서 reward 102.5→104.6(상승), ep_len 포화, 낙상 0, err_xy 0.93 안정 → **계속**; thermal 2.65→2.68 정체(3.0 초과 시 §7 조기 측정 조건 유지). **자동 스냅샷** 23:08 iter 5796 reward 104.8·DR 0.17·thermal 2.72 → 09-04 00:08 iter 6312 reward 105.0·err_xy 0.928·낙상 0·DR 0.22·thermal 2.80(완만 상승, 감시). 노트 §1b에 run_spec_tables.py 자동 표(액추에이터 한계·ROM/clip·PYG 플래그) 추가, P2 게인·clip = P1 동일 확인. **게이트 11~19(09-04 01:15~09:10) 전부 계속**: DR 0.27→0.69 램프 동안 reward 105.8→107.5(50avg 108.2 플래토), 낙상 0.000 유지, err_xy 0.89~0.97(v1 동일 단계 1.2 대비 우세), σ 0.247→0.213, thermal 2.69~2.89 등락(3.0 감시선 미도달). **10:09 스냅샷** iter 11565/18099(64%), reward 108.1, DR 0.75, thermal 2.85. 완주 ETA 09-04 저녁. 완주 게이트 = 운동학 재측정(무릎 swing 피크 vs 인간 55~65°) → 미달 시 swing knee 항 A/B · **완주 측정(09-05)**: 무릎 스윙 최대굴곡 좌 63.5/70.2° · 우 49.5/54.1°(0.6/1.2 m/s, 사람 55~65, 수정 전 0~10°) — 가설 확인; 좌우 비대칭 14°(발목 크랭크 토크 우 1.4~1.5배); 저속 정체 0.5 m/s 26 %, 회전 ±1.0 50 %; 모터 실효 전부 정격 안(엉덩이 벌림 76 %/95 °C 최대), 피크 초과 표시는 클립 접촉 2~4샘플 |
| **legonly_ab_v3_keyfix** [[2026-09-05_legonly_ab_v3_keyfix]] | v2와 **유일 변인 = 시작 자세 수정**(v30 크랭크 축 미러에 맞춰 웅크림 키프레임 재풀이: L_crank_A/R_crank_B 부호 정정, base z 0.868→0.904 m, 발바닥 −31→+5 mm, 폐루프 찢김 37.23→0.00 mm). 보상·설정 33개 동일. 질문: 좌우 비대칭(무릎 14°, 크랭크 토크 1.4~1.5배)이 줄어드는가 | 2026-09-05 발사, 진행 중 — 게이트 1(iter 724): reward 108.8, 낙상 0, 오차 0.470(v2 동시점 0.586) 계속; 게이트 2(iter 1479): 109.5, 낙상 0, 단계 1 진입; 게이트 3(iter 2233): 105.8, 낙상 0, 단계 2; 게이트 4(iter 2988): 102.5, 낙상 0, 단계 3; 게이트 5(iter 3748): 101.0, 낙상 0, 마지막 단계 4(vx≤2.5); 게이트 6(iter 4513): 100.3, 낙상 0, 졸업 비율 1.14(1.1 필요); 게이트 7(iter 5274): 102.9, 낙상 0, 비율 1.17(최대 체류로 졸업 예상); 게이트 8(iter 6039): 101.2, 낙상 0, 체류 2950/3000 → 2단계 진입 직전 |
| **legonly_ab_sideaware_smoke** [[2026-09-03_legonly_ab_sideaware_smoke]] | side-aware 픽스 스모크(1024 env, P1 200+P2 199) | 낙상 0·크래시 0·17 min; L_knee qtarget 진폭 0→27~33°(v1 5600 대비), R_knee 클립 포화 92.7% 잔존(#2 리워드 갭) | ✅ 인프라+창 열림 PASS |
| **legonly_ab_v2_smoke** [[2026-09-03_legonly_ab_sideaware_smoke]] | `legonly_ab_v1` 중단 원인(v30 좌우 미러축 vs mjlab 단일 정규식 default/clip)을 **설정·코드 레이어에서만** 수정한 뒤의 검증 스모크(1024 env). 리워드 가중치·항 변경 0건. 수정 3건 = per-side default(`signed_pose`, range 장축에서 부호 유도) + 관절별 action clip(`safe_target_clip`) + 폐루프 bent 키프레임 축 재표현(`_reexpress_loop_pose`). DR JSON은 본런과 동일 fastener50 | 프리플라이트 게이트 **수정 전 FAIL 2관절**(L_knee 사용창 0.00°, R_hip_pitch 42.75°<43.50 필요) → **수정 후 12관절 PASS**(L_knee 108.00°, R_hip_pitch 130.50°, L_hip_roll 44.5→99.0°, 좌우 완전대칭). 발목 폐루프 리셋 closure **37.27 mm → 0.001 mm**. P1 settle iter 154, P2 DR 1.000, iter 399 종료, 17분 01초, fell 0.000, 크래시 0. 무릎 창 열림(model_399, 0.6/1.2 m/s): L_knee qtarget 진폭 **0.00°→27.45/32.84°**·사용ROM 0.21→**30.14/37.21°**·스톱 상시토크 21.8→6.4/10.2 N·m, R_knee 진폭 0.00→3.95/5.97°·ROM 9.16/10.02° | ✅ **완주 — 인프라 PASS + 액션창 열림 확인**(보행 성능 판정 대상 아님). 회귀 안전성: v2/v3/v4 모델의 런타임 `default_joint_pos` **Δ 0.0000e+00°**, clip Δ 2.67e−04°(도↔라디안 왕복오차뿐, `--legacy-equivalence` 상시 검사). ⚠잔존(픽스 대상 아님): R_knee 명령이 여전히 93 % 클립 상한 고착 = 근본원인 §1 2순위(swing 무릎 리워드 부재), 별도 트랙. ⚠미해결: `shoulder_roll` 부호 규약 코드 2곳 모순(상체 런 전용, 근본원인 노트 §3c) |
