...
> **갱신 (2026-08-24)**: 이 문서는 "내부 부품을 PLA로 쓰면?"의 가정 분석이다. 이후 사용자 확인
> (2026-08-23, 커밋 `7f6c799`)으로 **푸시로드 `Arm_A`/`Arm_B`(긴 바)는 이미 알루미늄 가공품**임이
> 확정됐다 — 아래 순위의 **2위(로드 A 좌굴 0.14)·4위(로드 인장 0.33/0.38)·5위(로드 B 좌굴 0.45)는
> 실물에서 이미 해소**. 따라서 **실제 남은 가공 대기열은 ① 스토퍼 패드 ② 크랭크 A/B ③ 십자 핀
> ④ 클레비스 포크 ⑤ 탭 M5**이다.
