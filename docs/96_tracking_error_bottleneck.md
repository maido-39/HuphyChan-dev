...
| 학습 로그 값 | 0.69 | 나머지 ≈ 0.31 = 관측노이즈 + 에피소드 리셋 + 박스 모서리 표본 |
...
## 5. 그래서 무엇을 할 것인가
1. **지표 해석 교정**: 0.69는 배포 추종 오차가 아니다. 논문·리포트에는 결정론적 정상상태 **0.17 (AB) / 0.24 (RP)**와 전환 포함 0.26/0.34를 쓰고, 학습 곡선의 0.69는 "탐색 노이즈 포함 학습 지표"로 명시한다.
