...
6. ⚠**L5e 깨끗한 쌍 재시도 중** — 원거리 오염(19.94→29.91)의 원인은 `PYG_MAX_NODES` 420k
   내부 상한이었다(탄성 하우징 메시가 비싸 상한에 걸림). 800k로 올려 `L5eh_hip_elastic_clean`
   재실행 중. L5e 자체는 필드 SF 4.22로 여유가 커 판정에는 영향 없다.
