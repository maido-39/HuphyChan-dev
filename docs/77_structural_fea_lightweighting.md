...
## §25 판정 기준 정정 — 미달 수치의 대부분은 **필렛 없는 코너**였다 (2026-08-18)

도구: `tools/fea/convergence.py`(수렴 분류) · `tools/fea/verdicts.py`(이중기준 판정표)
그림: `docs/img/fea_mesh_convergence.png`

![메시 수렴성](img/fea_mesh_convergence.png)

*왼쪽 3패널: 같은 케이스를 핫스팟 국부 요소크기 $h$만 줄여 다시 푼 결과(왼→오 = 조밀해짐).
**점 최대(빨강)는 $\sigma \sim h^{-\lambda}$로 올라가는데 필드 p99(파랑)는 제자리**다.
오른쪽: 전 링크를 필드 기준으로 세운 판정 — 최악이 L5c 2.06으로 **전부 SF 2 이상**.*

### §25a 두 기준이 서로 다른 답을 준다

| 기준 | 정의 | 세분화에 대한 거동 |
|---|---|---|
| **점**(`max_vM_design`) | 하중주입·구속 절점 이웃을 뺀 최악 절점 | **발산** — 발 $h$ 3.97→1.50 mm에서 202→327 MPa |
| **필드**(`p99_vM`) | 전 절점 99 퍼센타일 | **안정** — 같은 구간에서 95.9→100.0 (+4 %) |

| 계열 | $h$ [mm] | 점 [MPa] | 필드 p99 | 점 변화 | **지수 $\lambda$** | 분류 |
|---|---|---|---|---|---|---|
| 발 toe-off | 3.97→2.60→1.50 | 202.2→249.6→**327.0** | 95.9→96.0→100.0 | **+62 %** | **+0.49** | **특이점** |
| 힙 pitch/roll | 3.38→1.40 | 310.2→**444.3** | 106.1→103.1 | **+43 %** | **+0.41** | **특이점** |
| 정강이 코너 | 5.41→2.00 | 135.7→143.6 | 36.4→36.5 | +6 % | +0.06 | **수렴** |

$\lambda \approx 0.5$는 **날카로운 재진입 코너**의 고전적 지수($\sigma \sim r^{-1/2}$)다. 발과 힙의
"미달"은 단면이 얇아서가 아니라 **필렛이 없어서** 나온 값이고, 메시를 더 조이면 얼마든지 커진다 —
**그 숫자로 내린 판정은 부재가 아니라 메시에 대한 판정**이다. 정강이 코너는 2.7배 세분화에도
+6 %라 수렴했으므로 그 143.6 MPa는 **실제 국부 응력**이다.

이는 이 프로젝트의 판정 3원칙([[feedback-fea-judgement-rules]]) 중 "특이점은 **초과 절점 수**로
판별한다"와도 일치한다 — 항복 초과 절점이 **0.001~0.007 %**(수십~수백 절점)에 불과하다.

### §25b 갱신된 판정표 (rev 2026-08-17j, 20 케이스)

| 링크 | 점 [MPa] | SF 점 | 필드 p99 | **SF 필드** | 항복 초과 절점 | 판정 |
| L1_ankle_foot | 6.8 | 40.64 | 3.6 | **76.82** | — | PASS |
| L1b_foot_toeoff | 202.2 | 1.37 | 95.9 | **2.88** | — | **필렛** |
| L1d_foot_toeoff_fine | 249.6 | 1.11 | 96.0 | **2.87** | — | **필렛** |
| L1e_foot_toeoff_finer | 327.0 | 0.84 | 100.0 | **2.76** | 0.002 % | **필렛** |
| L1f_foot_lateral_edge | 112.5 | 2.45 | 39.0 | **7.08** | — | PASS |
| L1g_foot_corner | 275.0 | 1.00 | 121.5 | **2.27** | — | 필렛?(미검증) |
| L2_shin | 135.7 | 2.03 | 36.4 | **7.58** | — | PASS |
| L2b_shin_cornerfine | 143.6 | 1.92 | 36.5 | **7.56** | — | 주의(수렴·실하중) |
| L2c_shin_nomotor | 194.0 | 1.42 | 39.1 | **7.06** | — | 필렛?(미검증) |
| L2e_shin_elastic | 927.5 | 0.30 | 53.6 | **5.15** | 0.004 % | 필렛?(미검증) |
| L3_thigh | 109.7 | 2.51 | 33.9 | **8.14** | — | PASS |
| L3c_thigh_nomotor | 607.5 | 0.45 | 90.7 | **3.04** | 0.006 % | 필렛?(미검증) |
| L3e_thigh_elastic | 111.9 | 2.47 | 35.6 | **7.75** | — | PASS |
| L4_hip_yaw | 130.9 | 2.11 | 34.8 | **7.94** | — | PASS |
| L5_hip_pitchroll | 310.2 | 0.89 | 106.1 | **2.60** | 0.004 % | **필렛** |
| L5c_hip_nomotor | 301.6 | 0.92 | 133.8 | **2.06** | 0.002 % | 필렛?(미검증) |
| L5d_hip_peakfine | 312.3 | 0.88 | 103.1 | **2.68** | 0.003 % | **필렛** |
| L5e_hip_elastic | 410.3 | 0.67 | 79.5 | **3.47** | 0.007 % | 필렛?(미검증) |
| L6_pelvis | 426.7 | 0.65 | 45.0 | **6.14** | 0.001 % | 필렛?(미검증) |
| L6c_pelvis_nomotor | 522.4 | 0.53 | 31.7 | **8.70** | 0.002 % | 필렛?(미검증) |

6061-T6 allowable: 276 (SF>1) / 184 (SF>1.5) / 138 MPa (SF>2)

**판정 라벨**: `PASS` = 점 기준으로도 SF≥2 · `필렛` = 점은 미달이나 **세분화로 특이점이 증명됨**
(필렛 R≥3 추가가 처방, 살 붙이기 아님) · `필렛?(미검증)` = 같은 양상이나 **세분화 변형이 아직 없어
증명 안 됨** · `주의(수렴·실하중)` = 점 값이 수렴했으므로 실제 국부 응력.

★**전 링크가 필드 기준 SF ≥ 2.06**이다. 이전 보고의 "L5 0.79 / L6 0.65 미달"은 점 기준값이었고,
그 점들은 모두 특이점이거나(증명됨) 특이점으로 의심되는(미증명) 위치다.

⚠**남은 작업이 라벨에 그대로 적혀 있다** — `필렛?(미검증)` 7건(L1g·L2c·L2e·L3c·L5c·L5e·L6·L6c)은
세분화 변형을 만들어 $\lambda$를 재야 확정된다. 그 전까지 "전부 통과"라고 말할 수 없다.

⚠**표는 스냅샷이다** — 캠페인(`autorun.sh`)이 돌고 있어 값이 갱신된다(이번 회차에도 L5 점
348.0→310.2, L5d 444.3→312.3으로 바뀌었다). `verdicts.py` 재실행이 항상 최신이다.
