...
> 걸지 않으며 `size_far` 동일성을 assert한다.
>
> **✅깨끗한 발 쌍으로 재확인 완료** — `L1h_foot_toeoff_clean`(원거리 10.12 고정, h 3.97→1.50):
> 점 202.2 → **323.8 (+60 %), $\lambda$ +0.48**, 항복 4절점(0.0016 %). 오염 쌍의 +0.49와
> 사실상 동일하므로 **원거리 오염은 결론에 영향이 없었고, 발 특이점은 확정**이다.
> 체적가중 p99도 82.7 → **81.8 (−1.1 %)** 로 깨끗한 쌍에서 메시 독립이 성립한다.
>
> **오염의 진짜 출처**: `run_link_env.py`의 내부 상한 `PYG_MAX_NODES`(기본 **420,000**)다.
> 스펙에 `max_nodes`를 안 걸어도 메시가 이 상한을 넘으면 러너가 **원거리를 성기게** 만든다.
> 따라서 쌍둥이를 만들 때는 상한을 함께 올려야 한다 —
> `PYG_MAX_NODES=800000 run_link_env.py <twin>`. 구형 3단 발 계열(L1b/L1d/L1e)은
> `convergence.py`에서 **퇴역**시키고 L1b+L1h 쌍이 대체한다.

| 계열 | $h$ [mm] | 점 [MPa] | **$\lambda$** | **항복 초과** | 분류 |
|---|---|---|---|---|---|
| **발 toe-off (깨끗한 쌍)** | 3.97→1.50 | 202→**324** | **+0.48** | 4 절점 (0.0016 %) | ★**특이점(확정)** |
| ~~발 toe-off (구 오염쌍)~~ | 3.97→1.50 | 202→327 | +0.49 | 4 절점 | ⚠오염 — 위로 대체 |
...
1. **필렛 처방 2건 확정**: 발 toe-off $(-103.7,118.0,-834)$ · L6 골반 $(-0.1,132.5,60.0)$.
   둘 다 깨끗한 쌍에서 $\lambda$ +0.48 / +0.91로 증명됐다. **R≥3 필렛**, 살 붙이기 아님.
...
6. ⛔**L5e(탄성 하우징)는 깨끗한 쌍 생성 불가 — 종결**. 3회 시도 후 원인 확정:
   `PYG_MAX_NODES` 상한이 아니라 **메셔 자체가 실패**한다. 러너의 사다리가
   `size_far` 19.94에서 실패하고 3번째 시도(29.91)에서야 성공하며, 그 메시에서는
   `bore` 시트가 **25절점(점지지)** 으로 떨어져 경고가 뜬다 — 탄성 하우징의 불리언 트림
   형상이 그 조밀도를 감당 못 한다. **L5e는 미분류로 확정**하되, 필드 SF 4.22로 여유가 커
   판정에 영향이 없다. 필요해지면 하우징 형상 단순화가 선행돼야 한다.
