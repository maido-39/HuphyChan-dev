...
## §25 메시 수렴성 전수 검증 — 특이점은 **발 하나뿐**이었다 (2026-08-18)

도구: `tools/fea/make_refine_variant.py`(세분화 쌍둥이 생성) · `tools/fea/convergence.py`(분류) ·
`tools/fea/verdicts.py`(이중기준 판정표) · 그림 `docs/img/fea_mesh_convergence.png`

> ⚠**같은 날 오전 판(초판) 정정**: 초판은 "미달 수치 대부분이 필렛 없는 코너"라고 결론냈다.
> 근거였던 힙 계열 L5d의 444.3 MPa가 **리비전 j 재실행으로 312.3으로 내려가면서** 그 계열의
> 발산이 사라졌고($\lambda$ +0.41 → **+0.01**), 새로 만든 쌍둥이 5건도 전부 수렴으로 나왔다.
> **진짜 특이점은 발 toe-off 계열 하나**다. 나머지 미달은 **실제 국부 응력**이다.

### §25a 방법 — 같은 케이스, 핫스팟 요소크기만 바꾼 쌍둥이

`_refines` 태그를 단 변형을 만들어 하중·구속·타이·리비전을 전부 복사하고 **핫스팟 국부
요소크기 $h$만** 줄인다. 답이 바뀌면 원인이 메시로 특정된다. 점 최대가 $\sigma \sim h^{-\lambda}$로
계속 오르면($\lambda>0.2$) 기하학적 특이점이고, 수렴하면 실제 국부 응력이다.

### §25b 결과 — 7계열 중 특이점은 1건

| 계열 | $h$ [mm] | 점 [MPa] | 필드 p99 | **$\lambda$** | 분류 |
|---|---|---|---|---|---|
| **발 toe-off** | 3.97→2.60→1.50 | 202.2→249.6→**327.0** | 95.9→100.0 | **+0.49** | ★**특이점** |
| 힙 pitch/roll | 3.38→1.40 | 310.2→312.3 | 106.1→103.1 | +0.01 | 수렴 |
| 정강이 코너 | 5.41→2.00 | 135.7→143.6 | 36.4→36.5 | +0.06 | 수렴 |
| L5c 힙(모터無) | 3.38→1.40 | 301.6→298.9 | 133.8→**141.1** | −0.01 | 수렴 |
| L5e 힙(탄성) | 6.00→1.40 | 410.3→**278.9** | 79.5→92.4 | −0.27 | 수렴 |
| L3c 대퇴(모터無) | 16.0→1.50 | 607.5→577.4 | 90.7→99.4 | −0.02 | 수렴 |
| L6c 골반(모터無) | 17.0→2.00 | 522.4→**274.3** | 31.7→31.5 | −0.30 | 수렴 |

$\lambda \approx +0.49$는 날카로운 재진입 코너의 고전 지수($\sigma\sim r^{-1/2}$)다 — **발만**
해당한다. 반대로 **음의 $\lambda$**(L6c −0.30, L5e −0.27)는 **조대 메시가 과대평가**하고 있었다는
뜻이다: 응력집중부에 큰 요소를 놓으면 그 요소 하나가 과장된 값을 낸다. L6c 522→274, L5e 410→279로
**절반 가까이 떨어졌다**.

### §25c 갱신된 판정표 (rev 2026-08-17j, 25 케이스)

| link | joint | point [MPa] | SF point | field p99 | **SF field** | yield 초과 절점 | 판정 |
|---|---|---|---|---|---|---|---|
...
| L1g_foot_corner | 275.0 | 1.00 | 121.5 | **2.27** | — | ~~FAIL(수렴)~~ <span title="coarser mesh">← L1gf_foot_corner_fine</span> |
| L1gf_foot_corner_fine | 273.7 | 1.01 | 119.6 | **2.31** | — | **FAIL(수렴)** |
...
| L3c_thigh_nomotor | 607.5 | 0.45 | 90.7 | **3.04** | 0.006 % | ~~FAIL(수렴)~~ <span title="coarser mesh">← L3cf_thigh_nomotor_fine</span> |
| L3cf_thigh_nomotor_fine | 577.4 | 0.48 | 99.4 | **2.78** | 0.019 % | **FAIL(수렴)** |
| L3e_thigh_elastic | 111.9 | 2.47 | 35.6 | **7.75** | — | PASS |
| L4_hip_yaw | 130.9 | 2.11 | 34.8 | **7.94** | — | PASS |
| L5_hip_pitchroll | 310.2 | 0.89 | 106.1 | **2.60** | 0.004 % | ~~FAIL(수렴)~~ <span title="coarser mesh">← L5d_hip_peakfine</span> |
| L5c_hip_nomotor | 301.6 | 0.92 | 133.8 | **2.06** | 0.002 % | ~~FAIL(수렴)~~ <span title="coarser mesh">← L5cf_hip_nomotor_fine</span> |
| L5cf_hip_nomotor_fine | 298.9 | 0.92 | 141.1 | **1.96** | 0.003 % | **FAIL(수렴)** |
| L5d_hip_peakfine | 312.3 | 0.88 | 103.1 | **2.68** | 0.003 % | **FAIL(수렴)** |
| L5e_hip_elastic | 410.3 | 0.67 | 79.5 | **3.47** | 0.007 % | ~~FAIL(수렴)~~ <span title="coarser mesh">← L5ef_hip_elastic_fine</span> |
| L5ef_hip_elastic_fine | 278.9 | 0.99 | 92.4 | **2.99** | 0.001 % | **FAIL(수렴)** |
| L6_pelvis | 426.7 | 0.65 | 45.0 | **6.14** | 0.001 % | ~~필렛?(미검증)~~ <span title="coarser mesh">← L6f_pelvis_peakfine</span> |
| L6c_pelvis_nomotor | 522.4 | 0.53 | 31.7 | **8.70** | 0.002 % | ~~FAIL(수렴)~~ <span title="coarser mesh">← L6cf_pelvis_nomotor_fine</span> |
| L6cf_pelvis_nomotor_fine | 274.3 | 1.01 | 31.5 | **8.75** | — | **FAIL(수렴)** |


**라벨**: `PASS` 점 기준 SF≥2 · `필렛` **세분화로 특이점 증명됨**(필렛 R≥3, 살 붙이기 아님) ·
`FAIL(수렴)` **점 값이 수렴 = 실제 미달** · `주의(수렴·실하중)` 수렴한 실제 국부 응력, SF 1.5~2 ·
`필렛?(미검증)` 쌍둥이 미완 · ~~취소선~~ = **더 조밀한 메시로 대체된 행**(별도 판정으로 읽지 말 것).

### §25d 결론

- **필드 기준 최악은 L5c(모터無) SF 1.96** — 초판이 "전 링크 2.06 이상"이라고 한 것은 세분화 전
  값이었다. 지금은 **2 미만이 1건**이다.
- **점 기준 실제 미달**(수렴 확인): L5 계열 0.88~0.92 · L3c(모터無) 0.48 · L1g 발 코너 1.01 ·
  L6c(모터無) 1.01 · L5e(탄성) 0.99. **필렛으로 해결되지 않는다** — 단면·하중경로 문제다.
- **모터 브래킷이 있고 없고가 지배적**이다: L3 대퇴 2.51 vs L3c(모터無) 0.48로 **5배**.
  즉 "모터 하우징이 하중을 나눠 받는다"는 전제가 성립해야 통과다(docs/78 §4).
- **발만 필렛 처방**: L1b~L1e 계열. 위치는 $(-103.7, 118.0, -834)$ 부근 무필렛 코너.

⚠**미결**: `L6f_pelvis_peakfine`·`L2cf_shin_nomotor_fine` 2건 미완 — 완료 시 재분류 필요.
⚠**표는 스냅샷**이다. 캠페인이 돌면 값이 바뀐다(이번에도 L5d 444.3→312.3으로 결론이 뒤집혔다).
