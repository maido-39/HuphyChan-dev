...
> **URDF 임포터가 써 넣은 솔버 반복수 32/1** 이었다. 4/8 로 바꾸면 ×1.10 / ×1.14 로 닫힌다.
...
| **최대 착지력(peak)** | ⚠️ **기본 설정에서는 무효(×1.97)**, 4/8 또는 4/4 에서 **×1.10** | 아래 권고 설정 필요 |
| **하중 상승률(rate)** | ⚠️ **기본 설정에서는 무효(×2.92)**, 4/8 에서 **×1.14** | 위치 반복수 4 + 속도 반복수 8 |
| **속도 추종** | ✅ 해결 | `vx_err` 0.139 → 0.087(8/4) / 0.118(4/8), MuJoCo 자신은 0.140 |

**권고**: IsaacSim 측 하중 스터디 표준 설정 = **`solverPositionIterationCount=4`,
...
**미해결**
- 위치 반복수를 32→4 로 낮추면 관절 구속 오차(articulation joint error)가 커질 수 있다. 이번 스윕은
  발밑 힘만 봤고 관절 토크·구속 잔차는 측정하지 않았다. 하중 스터디에 Isaac 관절 토크를 쓸 계획이라면
  이것부터 측정할 것.
