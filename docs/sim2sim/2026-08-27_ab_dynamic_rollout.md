...
3. ~~**RP 이식의 팔 자세 오류.**~~ **재측정 완료** → [[2026-08-27_xengine_dynamic_rollout]] §정정.
   통제군(`PYG_ARM_ABD_DEG=0`)이 커밋된 행과 비트 동일함을 먼저 확인한 뒤 4/8·8/4 을 다시 쟀다.
   최대 착지력·충격량·보폭수는 ±2 % 안이지만 **하중 상승률이 4/8 에서 +23.4 %** 움직여
   RP 의 상승률 정합은 ×1.14 가 아니라 **×1.41** 이다. AB 쪽 수치는 처음부터 15° 로 측정됐다.
