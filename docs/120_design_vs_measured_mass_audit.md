...
## 0★. 정정 (2026-09-03 13:40, 사용자 답변 반영 — 아래 §0~§3의 상체 판정을 대체)

사용자 확인: ① 실측 23.25 kg에 **waist-yaw 모터 포함**(나사 일부 미장착 → 모델이 무거운 게 정상)
② 전신 계량 시 **PSU 미장착** ③ **넥·양팔 전부 장착** 상태.

| 부위 | 모델 (PSU 1.95 kg 제외해 계량 조건과 일치) | 실측 | 오차 | 판정 |
|---|---|---|---|---|
| 하체 (LegOnly) | 23.630 | 23.25 ± 0.5 | **+0.380 kg (+1.6 %)** | ✅ 변동 없음 — 나사 미장착(약 20 %)으로 설명 |
| 상체 (FullDoF−LegOnly) | 12.044 − 1.950 = **10.094** | 10.0 ± 0.7 | **+0.094 kg (+0.9 %)** | ✅ **거의 정확** — §3의 "+2 kg 플레이스홀더 재질" 가설은 **불필요**(PSU 부재가 전부) |
| 전신 | 35.674 − 1.950 = **33.724** | ≈33.25 | +0.47 kg (+1.4 %) | ✅ 합성 불확도(±0.7) 내 |

결론: **v30 모델은 실물과 1~2 % 안에서 일치**한다. 하체 잔차(+0.38)는 나사, 상체 잔차(+0.09)는
잡음 수준. §3의 넥 Steel(1.61 kg)·암 ABS(0.91 kg) 재질 의혹은 질량 데이터로는 **지지되지 않는다** —
그 부품들이 실제로 출력물이라면 −1.4~−2 kg 과대가 있어야 하는데 실측이 그걸 보여주지 않으므로,
(a) 실제로 금속/솔리드 부품이거나 (b) 미모델 질량(하네스·전장, DR의 trunk_mass_extra +10 % 항목)이
우연히 상쇄한 것. **잔여 확인 1건(경미)**: 넥 어셈블리·암 커넥터의 실제 재질 — (b)라면 링크별 COM/관성
분포가 틀릴 수 있어(총질량은 맞아도) 상체 사용 변형 학습 전 부품별 재질만 확인하면 된다.
§0 부수 발견(DR이 "실물이 가벼운 방향"을 안 감쌈)은 **그대로 유효** — 하체 실측/모델 0.984.
§4 권고 2(플레이스홀더 재질 재설정)는 "재질 확인" 항목으로 격하, 나머지 권고 유지.

---
*(아래 §0~§4는 정정 전 원문 — PSU 장착을 가정한 분석. 기록 보존용.)*
