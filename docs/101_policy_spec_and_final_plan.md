...
### 5d. ~~★ 모터 여유 — T-N 곡선 대비~~ → **정정됨 (2026-08-26)**
> 아래 표는 **발목축 토크를 모터 T-N 곡선에 직접 대조한 오류**다. RP도 모터는 크랭크에 있다.
> 모터축으로 올바로 사상하면 **AB 0.51–0.53 / RP 0.44–0.55로 사실상 동일**하고 포화는 양쪽 다 ≤0.03 %다.
> 검증·상세는 [[2026-08-24_ankleRP_c3]] 정정 블록. 살아남는 결론: 발목축 수요 p99 59–65 N·m는 **RS03 직결로는 불가**,
> 2-RSU의 ~1.6배 증폭이 있어야 크랭크당 40 N·m(util 0.5)로 감당된다.
...
**한 줄**: *에너지는 RP가 12 % 싸고, 충격은 무승부이며, 모터 여유는 사실상 동률이다(2026-08-26 정정). 두 arm의 차이는 하드웨어가 아니라 **행동공간**이므로, 선택 기준은 에너지·보행스타일·배포 편의다.*
...
4. **모터 여유**: ~~AB 압도적 우세~~ → **정정: 사실상 동률**(모터축 AB 0.51–0.53 / RP 0.44–0.55, 포화 ≤0.03 %). 두 arm이 같은 2-RSU 토크 한계를 쓰므로 당연한 결과. 하드웨어 결론은 "발목 직결 RS03 불가, 링키지 필요"로 남는다(유도).
