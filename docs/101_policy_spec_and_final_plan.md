...
### 5g-2. ★ 보행 스타일 정량 비교 (model_20000, 명령 1.2 m/s, **속도 1.05–1.35 m/s 사이클만** 추출)
인간 기준은 `refs/human_gait` 4개 데이터셋(101점 게이트 사이클). Moissenet 2019는 자유보행 1.157 m/s·48명·215시행이라 명령속도와 거의 일치한다.
⚠ 속도가 다르면 보행 파라미터 비교가 무효라, **두 arm 모두 1.05–1.35 m/s 구간 사이클만** 골라 평균했다(AB 17 사이클 / RP 32).

![보행 스타일](img/abrp_gait_style_gait20k.png)

| 시공간 지표 | AB | RP | 인간 @1.2 m/s |
|---|---|---|---|
| 케이던스 [steps/min] | 139 | **150** | ~110 |
| 스트라이드 시간 [s] | 0.864 | 0.801 | ~1.1 |
| 스트라이드 길이 [m] | **1.123** | 1.001 | ~1.35 |
| duty factor (입각 비율) | **0.498** | 0.547 | ~0.60 |
| 이중지지 [%] | 10.0 | 11.1 | ~20 |
| **보폭(좌우) [m]** | **0.224** | **0.175** | ~0.10 |
| 베이스 높이 [m] | 0.851 | 0.861 | — |
| CoM 상하 진폭 [mm] | 41 | 44 | 40–50 ✓ |

**AB vs RP 스타일 차이**
1. **AB는 넓게 걷는다** — 보폭 224 vs 175 mm(**+28 %**). 둘 다 인간(~100 mm)보다 훨씬 넓지만 AB가 특히 넓다.
2. **AB는 크고 느린 걸음, RP는 잘고 빠른 걸음** — 스트라이드 1.12 m @139 spm vs 1.00 m @150 spm.
3. **AB의 duty가 0.498로 0.5 미만** = 비행 구간이 생기는 영역(달리기 쪽). RP는 0.547로 확실한 보행.
4. **무릎**: AB가 스윙에서 더 깊게 굽힌다(−73° vs −65°), 가동범위도 23 vs 20°.
5. **엉덩이**: 궤적 모양은 같고 AB의 진폭이 크다(45 vs 31°).

**둘 다 인간과 다른 점 (공통)**
- **수직 GRF가 M자가 아니다.** 인간은 입각 초·말에 ~1.1 BW 두 봉우리와 중간 골(~0.7)을 만드는데, 두 arm 모두 **0.8 BW 평탄**이다. 무릎을 굽힌 채(스탠스에서도 −45~−50°) 상하 진동을 억제하며 걷는 "크라우치 워킹"의 전형이다.
- **발목 push-off가 없다.** 인간은 62 %에서 **−17° 저측굴곡**으로 밀어내는데, AB는 −4°, RP는 −1°에 그친다. 다만 **배측굴곡 상승은 AB가 인간과 거의 겹친다**(+12° vs 인간 +13°, RP는 +8°).
- **스윙 중 발목 자세가 틀리다.** 인간은 스윙에서 중립(~0°)으로 돌아오는데 두 arm 모두 +7~+15° 배측굴곡을 유지한다(발끝 걸림 회피 전략으로 보임).
- 케이던스가 인간보다 26–36 % 높고 스트라이드가 17–26 % 짧다 = **잘고 빠른 걸음**.

**인간 유사도 판정**: 발목 파형만 놓고 보면 **AB가 더 인간에 가깝다**(배측굴곡 진폭·push-off 깊이 모두). 시공간 지표는 **RP가 더 가깝다**(보폭이 인간에 가깝고 duty가 0.5 이상으로 보행 영역). 종합하면 **어느 쪽도 인간형 push-off를 못 하고 있고, 이것이 다음 세대의 가장 큰 개선 여지**다([[62_policy_reward_design_review]] §2 toe-off 스택).

### 5g-3. ⚠ AB의 유지-명령 stall (재발, 조사 중)
model_20000·명령 1.2 m/s 롤아웃에서 **AB가 1.25 m/s로 6초 정상 보행 후 0.02 m/s까지 완전 정지**했다(2 s 창 평균: 1.22→1.26→0.73→0.16→0.05→0.02→0.59). RP는 같은 조건에서 1.23–1.27을 20초 내내 유지했다.
- 같은 패턴이 12k 재측정(0.8 m/s)에서도 AB에 나타났고, 9.5k 유지-명령 프로브에서는 RP에 나타났다 — **단일 env 결정론 재생의 고분산**이라는 것이 지금까지의 해석이다([[95_soft_landing_prescription]] §7b).
- 그러나 12k 내장 평가기(96 에피소드, 0.8 m/s)에서는 **두 arm 모두 stall 0**이었다. 이번 것은 **1.2 m/s + 만배 DR**이라 조건이 다르다.
- ⇒ 평가기(12 env, 1.2 m/s, 20 s 고정)로 검증 중. 통계적으로 재현되면 AB의 실질적 결함이고, 안 되면 프로브 아티팩트다. **완주 전 반드시 결론 낼 것.**
