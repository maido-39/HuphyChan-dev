...
*v1 → v2 (레드팀 1차) → v2.1 (레드팀 2차) → **v3 (2026-08-22, ★Fusion 360 MCP 실측 반영)**.
정정 내역 §7, Fusion 대조 §9.*

> ★**v3 전환**: 질량·관성의 출처가 STEP(8/14 export)에서 **Fusion 360 라이브 모델**로 바뀌었다.
> 총질량 43.70 → **50.31 kg**. 관절 축은 Fusion 베어링 기하로 **전부 동일함이 확인**됐다.
...
## §9 ★Fusion 360 실측 (2026-08-22, MCP 연결 성공)

MCP(`localhost:27182`, SSH 터널)로 접속해 활성 문서에서 직접 읽었다.
도구: `tools/fusion/mcp_client.py`(MCP streamable-HTTP 최소 클라이언트) +
`tools/robot_model/massprops_fusion.py`(바디 523개 → 강체 재집계).

### §9a 문서가 다르다 — 그리고 상체가 있다

| | 캠페인 STEP | Fusion 라이브 |
|---|---|---|
| 문서 | `260814_HumanMesh_OMAKASE…v1` | **`260819_HumanMesh_wUpper_OMAKASE…v4`** |
| 범위 | 다리+골반 | **+ Torso · Neck · Arm_R · 어깨 액추에이터** |
| 바디 | — | 523개, 합 **33.639 kg**(그룹합과 정확히 일치) |

### §9b ★모터는 전부 자리표시자였다 — 확정

| 모터 | 체적 | Fusion 질량 | 밀도 | 재질 |
|---|---|---|---|---|
| RS04 | 198.5 cm³ | 1.558 kg | **7.85** | "Steel" |
| RS03 | 118.7 cm³ | 0.932 kg | **7.85** | "Steel" |
| Neck(RS02+RS00) | 310.4 cm³ | 2.436 kg | **7.85** | "Steel" |

밀도가 정확히 강철 7.85 = **체적 × 범용강 밀도**. [[82_final_design_mass_review]] §3의 판정이
**소스에서 확정**됐고, 카탈로그 치환(RS04 1.42 / RS03 0.88 / RS02 0.405 / RS00 0.310)이 옳다.

### §9c ★관절 축 — Fusion 베어링 기하로 전부 확인, 변경 없음

| 관절 | 베어링 | 축 | 축선 |
|---|---|---|---|
| 힙 피치 | CRBS808AUUU-HipP2R | **x** | y 70.0, z 60.0 |
| 힙 롤 | 6810ZZ-HipR2Y | **y** | x −123.7, z 60.0 |
| 힙 요 | 6814ZZ-HipY2Knee | **z** | x −123.7, y 70.0 |
| 무릎 | 6810ZZ-HipY2Knee | **x** | y 115.0, z −310.0 |
| 발목 피치 | 6900ZZ-Knee2UJ | **x** | y 145.0, z −800.0 |
| 발목 롤 | 6900ZZ-UJ2Feet | **y** | x −123.7, z −800.0 |

세 힙 축이 **(−123.7, 70, 60)에서 교차** — v2 가정 그대로. 무릎 −310, 발목 −800도 동일.
**기하는 안 바뀌었고 질량만 바뀌었다.** (모터 프록시 COM은 최대 53 mm 다르지만 자리표시자라
관절 축 근거가 아니다 — 축은 베어링이 정한다.)

### §9d 강체 질량: STEP vs Fusion

| 강체 | STEP | **Fusion** | 차이 |
|---|---|---|---|
| 골반 | 5.653 | **5.371** | −5.0 % |
| 힙피치링크 | 2.178 | **2.172** | −0.3 % |
| 힙롤링크 | 1.820 | **1.609** | −11.6 % |
| 대퇴 | 2.756 | **2.896** | +5.1 % |
| 정강이 | 3.353 | **3.627** | +8.2 % |
| 발목 십자 | 0.087 | **0.087** | +0.3 % |
| 발 | 0.775 | **1.033** | **+33.2 %** |
| **상체** | 16.11(자리표시자) | **22.09**(실측, 팔 미러) | **+37 %** |
| **총계** | 43.70 | **50.31 kg** | +15 % |

STEP이 그룹별로 최대 24 % 가벼웠던 것은 빠진 나사 141개 때문이고, Fusion 값이 정답이다.

⚠**계산 중 잡은 함정**: 대퇴의 `CenterPin_RS03`라는 **핀 2개**가 이름에 "RS03"을 포함해
모터 카탈로그 치환에 걸려 **+1.76 kg 유령질량**이 붙었다. `family()`에 `'Robstride' in name`
가드를 넣어 수정(`massprops_fusion.py`). 이름 부분문자열 매칭의 전형적 사고.

### §9e 재검증 결과 (Fusion 데이터로)

| 항목 | 결과 |
|---|---|
| 링크 길이 | 힙→무릎 0.370 · 무릎→발목 0.490 · 힙→밑창 0.903 · 스탠스폭 0.2474 m — 전부 CAD 일치 |
| 좌우 규약 | 구 모델과 6관절 전부 일치(발 회전벡터 지문) |
| 관절 스윕 | 자기접촉은 내전 11.0° · 굴곡 −120°뿐, 나머지 4관절 0 |
| **관성(MuJoCo 질량행렬)** | 힙피치 축 **2.571**, 무릎 축 **0.536 kg·m²** (점질량 추정 2.482 / 0.484) |
