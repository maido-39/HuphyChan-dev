...
*v1 → **v2 (레드팀 13에이전트 반영)**: 6개 주장 중 massprops·frames·placeholders REFUTED,
env-binding DISPUTED, inertial-transform·meshes STANDS. 정정 내역은 §7.*
...
2. **강체 귀속 (v2 정정)**: 볼트 패턴(`interface_review`)을 따른다 — 골반 = CenterParts + Waist_Yaw RS04
   ...
   결과: 골반 5.48 / 힙피치 2.18 / 힙롤 1.82 / **대퇴 2.76 / 정강이 3.36** / 십자 0.07 / 발 0.79 kg.
...
3. **관절 프레임**: 힙 3축이 **(−123.7, 70, 60)에서 교차**(모터 원통축 실측 — 구 MJCF의 힙롤 15° 캔트는
   ...
   knee −y, ankle_pitch −y, ankle_roll −x; **오른다리는 구 모델처럼 두 롤 축만 미러** — +q = 양쪽 모두 내전/
   내번, 요는 양쪽 같은 방향). `validate_robot.py`가 구 모델과 관절별 좌우 규약 일치를 assert한다.
...
5. **빌드 — `build_robot.py`**: 같은 JSON에서 URDF와 MJCF를 동시에 생성(일관성 보장), MuJoCo 컴파일 검사.
   충돌 프리미티브 24개: 몸통·머리·골반 박스, 힙피치 구(r 68)·힙롤 캡슐·대퇴·정강이·**정강이 뒤 발목모터 쌍
   캡슐**·발 캡슐 5열/다리. 골반–힙롤링크, 힙피치링크–대퇴는 구조상 중첩된 클러스터라 제외. 볼록껍질은
   `hull` 클래스(기본 비활성). mjlab 제약 예산(njmax 1500)에 맞춰 모터 원통은 충돌에서 뺐다.
...
| 관절 스윕 자기충돌 | hip_roll 내전 ≥13° L/R 정강이(다른 다리 직립 시), hip_pitch −125° 대퇴–몸통 5 mm — 둘 다 기하학적 사실 | 나머지 4관절 0 |
| **관성 실측(MuJoCo 질량행렬)** | 힙피치 축 **2.261 kg·m²**, 무릎 축 **0.451** | STEP 점질량 추정 2.214 / 0.437 (+2 %) ✓ |
| 총질량 | **43.53 kg** | 표 교정값 44.51(배터리 제외) — 차이 ≈ 나사 141개/다리(~0.8 kg)·CAD 리비전 차 |
...
2. **모터 질량 = 카탈로그**(케이블·브래킷 제외). → Fusion 실측. 양쪽 힙피치 스테이터 모두 골반에 계상(v1은
   오른쪽 1.42 kg 누락, base COM이 2.5 mm 좌편향이었음 → 지금 0.4 mm).
...
태스크 `thermal_effort` 정격 · URDF velocity 20 rad/s 출처(RS04 무부하 19.9).
