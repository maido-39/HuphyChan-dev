...
| 4 | H8 물리성립: 클레비스 볼트방향=발 좌우축·RP_h 스택·요크 ROM·~~스토퍼 패드 경로~~ | CAD 확인 | 사용자 CAD — **스토퍼 하중·단면 요구는 §12에서 정량 종결**(반경 60 mm·전단 62 mm², 크랭크측 금지). 남은 건 CAD에서 패드 경로 **확보 여부**뿐 |
...
가운데: 크랭크측 요동각 — 전 영역 5~8°로 한계의 1/3에 불과하다. 오른쪽: 크랭크 A 각도,
...
## §12 하드 스토퍼 — 하중 기준 정정, AB(크랭크) vs RP(짐벌), 요구 단면적 (2026-08-17, 사용자 요청)

도구: `tools/ankle_stop_residual.py`(하중 기준) · `tools/ankle_stopper_sizing.py`(사이징, 2단 자기검증)
그림: `docs/img/ankle_stop_residual.png` · `docs/img/ankle_stopper_sizing.png`

> **초안 정정 이력** — 이 절의 첫 판(같은 날 오전)은 하중 기준을 실측 **총 관절모멘트**
> (pitch 152.1 / roll 44.4 N·m)로 잡았다. 그건 틀렸다: 충격 순간 제약모멘트와 모터토크가
> **반대 부호**여서 스토퍼가 흡수하는 잔여량이 관절모멘트 자체보다 **크다**. 아래는 실측
> 스톱잔여모멘트 기준으로 전면 재계산한 값이며, **요구 단면적이 약 2배** 커졌다.

### §12a 하중 기준 — 두 선행값이 모두 틀렸다

![스톱잔여모멘트 하중기준 정정](img/ankle_stop_residual.png)

*좌·중: docs/64 §8e(빨강)와 §8i 보정본(파랑). §8e의 pitch P99 213 / peak 1056은 보정 후
**45 / 280 = 21 % / 26 %**로 내려간다. 우: 실제 설계 기준 — 관절이 **캡에 닿아 있는 프레임만**
골라낸 통계. pitch 상시 P99 143 · 설계 peak **280 N·m**, roll 89 · **93 N·m**이고,
**pitch는 전 프레임의 7.8 %, roll은 6.5 %가 캡 접촉**이다.*

스토퍼가 받는 것은 관절모멘트 전체가 아니라 **모터가 만들지 않은 몫**이다(docs/64 §8e 정의):

$$M_{stop} = M_{constraint} - \tau_{motor}$$

**선행값 ①: docs/64 §8e (2026-07-24) — 기준점 버그 포함.**
§8e는 pitch P99 213 / raw peak **1056 N·m**, roll 135 / 554를 기록했고, 이것이 하드스톱을
십자핀 경로 밖으로 뺀 근거였다. 그런데 §8e는 §8i(2026-08-04, 링크 wrench 모멘트가 관절이
아니라 **로봇 CoM** 기준이라는 발견)보다 10일 앞선다. 이웃 절 §8c·§8d·§8f는 모두
"과대평가" 경고를 달았지만 **§8e만 경고가 누락**됐다. 같은 롤아웃(`rough.npz`)에
$M_{joint} = M_{com} + (p_{com}-p_{joint})\times F$ 전송을 적용해 재계산한 결과가 위 그림이며,
§8e 값은 **P99의 21 % · peak의 18~26 %**로 내려간다. → **§8e 수치는 폐기, 본 절이 대체.**

**선행값 ②: 본 절 초안 — 총 관절모멘트.** `moments_8i.json`의 pitch peak 152.1 / roll 44.4를
썼는데, $M_{stop}$은 이보다 **크다**(pitch 280 vs 152). 부호가 반대인 순간이 지배하기 때문.

**확정 기준(위 그림 우측 패널)** — 7개 롤아웃 × L/R, **캡 접촉 프레임만**:

| 축 | 상시(on-limit P99) | 설계(P99×1.25) | **설계 peak** | 최악 출처 | 캡 접촉 비율 |
|---|---|---|---|---|---|
| **pitch** | 142.7 | 178.4 | **279.6 N·m** | bent_fcp / L | **7.8 %** |
| **roll** | 89.3 | 111.6 | **93.3 N·m** | flat25p1_fcp / L | **6.5 %** |

**★스토퍼는 abuse 부재가 아니다.** 관절이 시간의 **6~8 %를 캡에 붙어** 있다 —
docs/72 §3d의 경고("ROM 캡이 사용 밀집대역에 접근할수록 스토퍼 접촉이 일상 하중이 됨")가
정확했다. 따라서 정적 강도뿐 아니라 **마모·피로**가 판정에 들어가야 한다(§12e).

⚠**정책 품질 편차가 크다**: 현세대 gen21은 pitch peak 49~67 · roll 2.8~14.5로 얌전한 반면,
구세대 flat25p1(P1·no-DR)은 roll이 −105.8°까지 튀며 peak 93.3을 만든다. 위 포락은
구세대를 포함한 **보수 포락**이다. gen21만 쓰면 pitch 67 / roll 15로 4~6배 완화되지만,
docs/62 최악조건 프로토콜(수렴만·in-DR·멀티시드)에 따른 재측정 전까지는 보수측을 채택한다.

### §12b 두 스토퍼는 같은 한계의 두 좌표 — 야코비안으로 연결

AB(크랭크 $\phi_A,\phi_B$)와 RP(발 pitch/roll)는 독립 설계항목이 아니라
$J = \partial(\phi_A,\phi_B)/\partial(pitch,roll)$ 로 묶인 **같은 한계의 두 표현**이다.
가상일에 의해

$$M_{joint} = J^{\mathsf T}\,\tau_{crank}, \qquad \tau_{crank} = J^{-\mathsf T} M_{joint}$$

이므로 한쪽 토크를 알면 다른 쪽은 계산값이지 가정값이 아니다. $J$는 §10c 크랭크각을
재현하는 폐형식 기구학(`ankle_rom_map.geom`)을 중심차분해서 얻는다.

**검증 2단 (툴이 매 실행 강제, 실패 시 `SystemExit`)**
1. `selfcheck()` — 폐형식 크랭크각이 §10c 기록값을 0.05° 이내 재현
2. `selfcheck_statics()` — 차분 야코비안이 **명시적 로드 정역학**(로드 단위벡터 → 크랭크
   토크평형으로 로드력 → RP 중심 모멘트, 차분 없음)과 14개 크랭크/자세 조합에서
   **1.7e-06 N·m** 일치

`ankle_stop_residual.py`도 자체 검증을 갖는다: 축방향 제약모멘트와 모터토크의 상관이
**+0.65~0.95**(부호규약을 가정하지 않고 데이터가 지지하는 쪽을 선택) — 캡에 닿지 않은
구간에서 잔여가 0에 수렴해야 한다는 요구와 일치한다.

### §12c 답 — 스토퍼 하중과 요구 단면적

![스토퍼 하중과 요구 단면적](img/ankle_stopper_sizing.png)

*좌: 스토퍼 반경별 접촉면 하중. 중: SF 2.0 요구 단면적(실선=전단, 파선=베어링).
우: 설계 ROM 전역에서 설계 스톱모멘트가 역구동하는 크랭크 토크 — **최소 228 N·m조차 RS03
피크(60)의 3.8배**이고, 청록 별(pitch −50/roll −20)의 **338 N·m**가 최악.*

6061-T6, **SF 2.0**: 전단 허용 $\tau_a = 0.577\times276/2 = 80$ MPa,
베어링 허용 $\sigma_a = 1.5\times276/2 = 207$ MPa.

| 스토퍼 / 구동원 | 토크 | 반경 | **접촉면 힘** | **요구 A(전단)** | **요구 A(베어링)** |
|---|---|---|---|---|---|
| **AB, 지면 구동 ← 최악** | **338.0 N·m** | 20 mm | **16,898 N** | **212.2 mm²** | 81.6 mm² |
| | | 40 mm | 8,449 N | 106.1 mm² | 40.8 mm² |
| AB, 모터 스톨 | 60.0 N·m | 20 mm | 3,000 N | 37.7 mm² | 14.5 mm² |
| | | 40 mm | 1,500 N | 18.8 mm² | 7.2 mm² |
| **RP, 지면 구동 ← 채택 경로** | **294.8 N·m** | 40 mm | **7,369 N** | **92.5 mm²** | 35.6 mm² |
| | | 60 mm | 4,913 N | **61.7 mm²** | 23.7 mm² |
| RP, 모터 구동(양 모터 피크) | 113.7 N·m | 40 mm | 2,844 N | 35.7 mm² | 13.7 mm² |
| | | 60 mm | 1,896 N | 23.8 mm² | 9.2 mm² |

**전단이 항상 지배**한다(그림 중앙: 실선이 파선의 2.6배). $A = F/\tau_a$ 로 반경에
반비례하므로 **스토퍼를 반경 바깥으로 빼는 것이 단면을 키우는 것보다 싸다** —
RP를 40 → 60 mm로 옮기면 92.5 → 61.7 mm²다.

**최악 자세**: 지면 구동 최악은 **pitch −50° / roll −20°**(저굴+내반 코너, 그림 우측 청록 별)에서
크랭크 A에 338.0 N·m. 모터 구동 최악은 pitch +30 / roll +20에서 관절모멘트 113.7 N·m.

### §12d ★ 판정 — 스토퍼는 RP(짐벌)측, 크랭크측은 **금지**

이건 새 발견이 아니라 **docs/64 §8c-RT·§8e(2026-07-24)의 지시를 정량 재확인**한 것이다.
§8c-RT는 이미 "발목 기계 하드스톱을 로드가 아니라 짐벌/관절측에 배치"를 지시했고,
§8e는 "하드스톱은 십자핀 경로 밖, **발↔정강이 직접 접촉 패드**"로 구체화했다.
본 절은 그 지시가 **보정된 하중으로도 여전히 옳다**는 것과, 위반 시 무엇이 깨지는지를 준다.

1. **★로드엔드가 파단권** — 스토퍼가 **크랭크에** 있으면 과주행 하중 전체가 로드를 통과한다:
   $F_{rod} = 338.0\times10^3/65 = \mathbf{5{,}199\ N}$ → JS6 정적 안전율
   $s_0 = C_0/F = 5003/5199 = \mathbf{0.96}$ — 설계 하한 1.875는커녕 **정적 정격 자체를 초과**한다
   (§7-5의 $C_0 = 5{,}003$ N = 510 kgf 카탈로그값). 참고로 docs/64 §8c-RT가 버그 있는
   630 N·m에 다른 레버(2×60 mm)를 곱해 얻었던 "로드력 peak ~5.3 kN"과 **2 % 일치**한다 —
   경로는 달라도 결론(로드 경유 금지)은 같다.
   스토퍼가 **RP 짐벌에** 있으면 하중이 발 → 짐벌 → 스토퍼로 단락되어 로드는 보행 하중
   (P99 1.10 kN, $s_0$ 4.5)에 머문다.
2. **하중과 반경이 모두 유리** — RP 294.8 vs AB 338.0 N·m인데다 짐벌은 반경을 크게 잡을 수
   있다(40~60 mm 대 크랭크 20~40 mm). 접촉면 힘 **4,913 N vs 16,898 N = 3.4배 차이**.
3. **모터는 어차피 못 버틴다** — 그림 우측 최소값 228 N·m조차 RS03 피크 60의 3.8배다.
   ROM 어디에서도 모터 토크로 캡 하중을 지탱할 수 없으므로 하드 스토퍼는 선택이 아니다.
4. **십자핀 경로도 금지** — §8e가 이미 계산: 스톱모멘트를 십자핀 커플로 받으면 span 40 mm에서
   peak 26 kN. 보정 하중(1056 → 280 N·m)으로 환산해도 **약 7 kN/핀**으로 여전히 불가.

**권고**:
- **RP 짐벌 = 발↔정강이 직접 접촉 패드**, 반경 **≥60 mm**, 전단 유효면적 **≥62 mm²**
  (예: 폭 16 × 두께 4 mm 리브 2곳, 또는 Ø10 강재 핀 2개). 접촉면은 6061 직접 접촉 대신
  **강재 인서트**를 쓰면 베어링 허용이 올라가 접촉폭을 줄일 수 있다.
- **우레탄 범퍼 병용**(§8e 지시): 위 값은 **강체 접촉 정적** 기준이다. 캡 접촉이 시간의
  6~8 %인 만큼 컴플라이언스로 접촉시간을 늘리면 peak가 **1/3~1/5**로 내려가고 마모·소음도
  잡힌다. 에너지 기반 범퍼 사이징은 미착수(§12e).
- **AB측 크랭크 스토퍼는 2차(모터 폴트 전용)로만** — 모터 스톨 60 N·m만 받으면 반경 40 mm에서
  19 mm²로 충분. 단 **RP 스토퍼가 반드시 먼저 닿도록** 각도 여유를 배분해야 로드엔드가
  1번 항의 파단권에 들어가지 않는다.
...
| # | 항목 | 성격 |
|---|---|---|
| 1 | **우레탄 범퍼 에너지 사이징 미착수** — 위 단면적은 강체 접촉 정적값. 실제 peak는 컴플라이언스에 의존 | 미계산 |
| 2 | **마모·피로 미판정** — 캡 접촉 6~8 %는 상시 하중이므로 접촉면 마모수명과 패드 피로가 필요 | 미계산 |
| 3 | **sim ROM ≠ 설계 ROM** — sim은 pitch −50..**+40**인데 설계는 −50..**+30**. 배굴캡이 10° 더 조이면 접촉 빈도·하중이 **증가**한다(현 포락은 그만큼 비보수) | 재측정 필요 |
| 4 | **구세대 정책 포함 포락** — flat25p1(P1·no-DR)이 roll peak를 지배. docs/62 프로토콜(수렴·in-DR·멀티시드) 재측정 시 완화 여지 | 재측정 |
| 5 | **소프트 리밋 관통** — MuJoCo 관절리밋은 연성이라 실제 travel이 캡을 넘는다(roll −105.8° 등). 강체 스토퍼는 그 에너지를 대신 받는다 | 모델 한계 |
| 6 | 스토퍼 **굽힘**은 형상 의존이라 미포함(단면적 답에는 전단·베어링만) | 형상 확정 후 |

관련: [[56_humanoid_impact_fall_load_handling]]의 계층 스택(충격내성 전달계·과부하 정격·
슬립 클러치)과 같은 논리 — 스토퍼 하나로 최악을 버티게 하지 않는다.
