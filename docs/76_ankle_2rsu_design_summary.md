...
## §12 하드 스토퍼 — AB(크랭크)측 vs RP(짐벌)측 하중과 요구 단면적 (2026-08-17, 사용자 요청)

도구: `tools/ankle_stopper_sizing.py` (실행 시 2단 자기검증 통과 후 계산)
그림: `docs/img/ankle_stopper_sizing.png`

![스토퍼 하중과 요구 단면적](img/ankle_stopper_sizing.png)

*좌: 스토퍼 반경별 접촉면 하중. 중: SF 2.0에서 요구 단면적(실선=전단, 파선=베어링).
우: 설계 ROM 전 영역에서 실측 피크 발목 모멘트가 역구동하는 크랭크 토크 —
**최소값 118 N·m조차 RS03 피크(60)의 2.0배**라, 파란 별(pitch −50/roll −20)의 174 N·m가 최악.*

### §12a 두 스토퍼는 같은 한계의 두 좌표 — 야코비안으로 연결

AB(크랭크 φ_A, φ_B)와 RP(발 pitch/roll)는 독립 설계항목이 아니라
...
2. `selfcheck_statics()` — 차분 야코비안이 **명시적 로드 정역학**(로드 단위벡터 → 크랭크
   ...
   **1.7e-06 N·m** 일치. 불일치 시 `SystemExit`.

### §12b 스토퍼를 때리는 두 가지 원인 — 크기가 다르다

| 구동원 | 무엇인가 | 상한 |
|---|---|---|
| 모터 구동 | 제어 폴트로 액추에이터가 한계까지 밀어붙임 | RS03 피크 **60 N·m/모터** |
| **지면 구동** | 이미 한계각인 발을 지면이 때림(헛디딤·모서리 착지) | 실측 최악 발목 모멘트 **pitch 152.1 / roll 44.4 N·m** |

스토퍼는 보행 하중경로에 없는 **abuse 부재**이므로 P99가 아니라 **피크층**(docs/62)으로 사이징한다.
지면 구동이 크랭크를 역구동한 토크가 지배적이다 — 그림 우측 패널이 그 분포다.
...
|---|---|---|---|---|---|
| **AB, 지면 구동 ← 지배** | **174.2 N·m** | 20 mm | **8,712 N** | **109.4 mm²** | 42.1 mm² |
| | | 40 mm | 4,356 N | 54.7 mm² | 21.0 mm² |
| AB, 모터 스톨 | 60.0 N·m | 20 mm | 3,000 N | 37.7 mm² | 14.5 mm² |
| | | 40 mm | 1,500 N | 18.8 mm² | 7.2 mm² |
| **RP, 지면 구동** | **158.4 N·m** | 40 mm | **3,961 N** | **49.7 mm²** | 19.1 mm² |
| | | 60 mm | 2,641 N | 33.2 mm² | 12.8 mm² |
| RP, 모터 구동(양 모터 피크) | 113.7 N·m | 40 mm | 2,844 N | 35.7 mm² | 13.7 mm² |
| | | 60 mm | 1,896 N | 23.8 mm² | 9.2 mm² |

**전단이 항상 지배**한다(그림 중앙 패널: 실선이 파선의 2.6배). 요구 단면적은
$A = F/\tau_a$ 로 반경에 반비례하므로, 스토퍼를 **반경 바깥으로 빼는 것이 단면적을 키우는
것보다 싸다** — AB를 20 → 40 mm로 옮기면 109 → 55 mm²로 절반이 된다.

**최악 자세**: 지면 구동 최악은 **pitch −50° / roll −20°**(저굴+내반 코너, 그림 우측 파란 별)에서
크랭크 A에 174.2 N·m. 모터 구동 최악은 pitch +30 / roll +20에서 관절 모멘트 113.7 N·m.

### §12d ★ 설계 판정 — 스토퍼는 RP(짐벌)측에 두어야 한다

세 가지가 모두 같은 방향을 가리킨다.

1. **하중이 작다** — RP 158.4 vs AB 174.2 N·m, 게다가 RP 짐벌은 반경을 크게 잡을 수 있다
   (40~60 mm 대 크랭크 20~40 mm). 결과적으로 접촉면 힘 **2,641 N vs 8,712 N = 3.3배 차이**.
2. **★로드엔드를 보호한다** — 스토퍼가 **크랭크에** 있으면 과주행 하중 전체가 로드를 통과한다:
   $F_{rod} = 174.2\times10^3/65 = 2{,}681$ N → JS6 정적 안전율 $s_0 = C_0/F = 5003/2681 =
   \mathbf{1.87}$ 로 **설계 하한 1.875를 밑돈다**(§7-5의 $C_0 = 5{,}003$ N = 510 kgf 카탈로그값).
   스토퍼가 **RP 짐벌에** 있으면 하중이 발 → 짐벌 → 스토퍼로 단락되어 로드는 보행 하중
   (P99 1.10 kN, $s_0$ 4.5)에 머문다.
3. **모터는 어차피 못 버틴다** — 그림 우측 패널의 최소값 118 N·m조차 RS03 피크 60의 2.0배다.
   즉 ROM 어디에서도 **모터 토크로 피크 하중을 지탱할 수 없으므로**, 하드 스토퍼는 선택이
   아니라 필수이고 back-drive 경로를 최소화하는 배치가 정답이다.

**권고**: RP 짐벌 포크에 반경 **≥50 mm** 스토퍼, 전단 유효면적 **≥40 mm²**
(예: 폭 10 × 두께 4 mm 리브, 또는 Ø8 강재 핀 2개). 접촉면은 6061 직접 접촉 대신
**강재 인서트/핀**을 쓰면 베어링 허용이 2배 이상 올라가 접촉폭을 줄일 수 있다.

**AB측을 병용할 경우**(모터 폴트 전용 2차 스토퍼): 모터 스톨 60 N·m만 받으면 되므로
반경 40 mm에서 **19 mm²**로 충분하다. 단 이때도 지면 구동 하중이 RP 스토퍼에서 먼저 멎어야
로드엔드가 보호된다 — **RP 스토퍼가 AB 스토퍼보다 먼저 닿도록** 각도 여유를 배분할 것.

### §12e 한계

- 피크 모멘트 152.1/44.4 N·m는 **포락선**이다 — 두 축 최대값이 ROM 코너에서 동시에
  발생한다는 실측은 아니며, abuse 부재에 대한 보수적 적용이다.
- 정적 판정이다. 실제 스토퍼 타격은 충격이므로 **탄성 스토퍼/댐퍼**로 접촉시간을 늘리지
  않으면 위 힘이 배 이상 커질 수 있다 — [[56_humanoid_impact_fall_load_handling]]의
  계층 스택(충격내성 전달계·과부하 정격·슬립 클러치)과 같은 논리.
- 스토퍼 **굽힘**은 형상 의존이라 여기서 다루지 않았다(단면적 답에는 전단·베어링만 포함).
  보스 형상이 정해지면 근원 굽힘을 별도 검토할 것.
