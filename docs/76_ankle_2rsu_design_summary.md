...
| 4 | H8 물리성립: 클레비스 볼트방향=발 좌우축·RP_h 스택·요크 ROM·~~스토퍼 패드 경로~~ | CAD 확인 | 사용자 CAD — **스토퍼 하중·단면 요구는 정량 종결**(docs/78 §13: 롤 짐벌 반경 ≥60 mm·전단 ≥86 mm², 크랭크측은 로드엔드 $s_0$ 0.54로 금지). 남은 건 CAD에서 **발↔정강이 패드 경로 확보 여부**뿐 |
...
수 배라 **모터로는 캡 하중을 지탱할 수 없다**(하드 스토퍼는 선택이 아님).*
...
| 축 | 상시(on-limit P99) | **설계 peak** | 캡 접촉 비율 |
|---|---|---|---|
| **pitch** | 172.5 | **379.0 N·m** | **9.5 %** |
| **roll** | 82.1 | **153.8 N·m** | **7.7 %** |
...
| **✅롤 짐벌, 지면 구동 ← 채택** | **409.0 N·m** | **60 mm** | **6,817 N** | **85.6 mm²** | 32.9 mm² |
...
$$F_{rod} = \frac{429.8\times10^3}{46.1} = 9{,}325\ \text{N}
...
반경 **≥60 mm** · 전단 유효면적 **≥86 mm²** · 우레탄 범퍼(Shore 90A) + 금속 하드스톱 2단.
...
에너지 기반 범퍼 사이징($\tfrac12 I\omega^2$) 미착수 · 접촉면 피로·마모 미판정 ·
sim ROM(pitch +40)이 설계 ROM(+30)보다 넓어 현 포락이 비보수 · 구세대 정책 포함 포락 ·
스토퍼 굽힘 미포함. 전체 표는 docs/78 §14.
