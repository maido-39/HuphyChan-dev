...
> 묻지 않았다. 실측 스톱잔여모멘트로 다시 잡으면 크랭크 토크는 **515.1 N·m(3.8배)**, 로드력은
> **9,620 N**으로 JS6 정격($C_0$ 5,003 N)을 **1.9배 초과**한다 → §13a.
...
**최소 336.3 N·m** — RS03의 **1초 과부하** 60 N·m의 5.6배, **연속 스톨 정격 13 N·m**(`Motor_Spec/RS03_overload_stall.csv`)의 **25.9배**다. 스토퍼를 버티는 것은 스톨 조건이므로 비교 기준은 13이고, **모터로는 캡 하중을 지탱할 수 없다**.*
...
| **✅RP 짐벌(롤), 지면 구동 ← 채택** | **413.0 N·m**† | 40 mm | 10,326 N | 129.7 mm² | 49.9 mm² |
| | | **60 mm** | **6,884 N** | **86.5 mm²** | 33.3 mm² |
...
† $\text{hypot}(379.0,\,164.2)$ — **서로 다른 롤아웃·서로 다른 축**의 포락이라 단일 면이 실제로
받는 값이 아니다. 축별로 나누면 pitch 면 79.3 mm² · roll 면 34.4 mm²이고, 합성 86.5 mm²는
두 축을 한 부재가 함께 받을 때의 보수 상한이다.

**§10 대비**: 크랭크 토크 134 → **515.1 N·m (3.8배)**, 필요 패드면적 13.5 → 62~162 mm².
...
- **롤 짐벌 스톱 = 발↔정강이 직접 접촉 패드**(docs/64 §8e 지시: 십자핀 커플 경로 **금지** —
  span 40 mm에서 커플 하중 과대). 반경 **≥60 mm**, 전단 유효면적 **≥87 mm²**
...
### §13c ★ 설계 ROM에서는 스토퍼의 **성격 자체가 바뀐다** — 최대 미결

sim 모델의 pitch 캡은 **+40°** 인데 설계 ROM은 **+30°** 다. 학습된 보행이 설계캡을 얼마나
넘는지 직접 재면(31 롤아웃 × L/R = 62 채널, 전 프레임):

| 축 | 설계 ROM | 설계캡 **밖** 프레임 비율 (최악 / 중앙) |
|---|---|---|
| **pitch** | −50…**+30** | **33.0 %**(rolloff30_p2_fc/R) / **16.6 %** |
| roll | −20…+20 | 4.3 %(rough/R) / 0.09 % |

즉 **설계캡 +30을 실제로 걸면 pitch 스토퍼는 4~9 % 듀티 부재가 아니라 보행 모멘트를
프레임의 1/6~1/3 동안 받는 상시 구조부재**가 된다. 그러면 본 절의 정적 판정 프레임 전체가
**마모·연속접촉 해석**으로 대체돼야 하고, 스토퍼는 사실상 감속기 출력단의 일부가 된다.

→ **§12·§13은 그때까지 잠정이다.** 두 갈래 중 하나를 골라야 한다:
1. **설계캡을 +40으로 올린다** — 기구가 배굴 +40을 낼 수 있는지 docs/76 §11 커버리지로 재확인
   (현 설계 박스는 +30까지만 검증됨), 또는
2. **+30 캡을 실제로 걸고 재학습·재측정한다** — 정책이 캡에 맞춰 배굴을 줄이면 이 문제가
   사라질 수도, 캡에 계속 부딪히며 하중이 더 커질 수도 있다. **어느 쪽인지는 측정 전에는 모른다.**

roll은 설계 ±20 = sim ±20이라 해당 없다(위 4.3 %는 MuJoCo 연성리밋 관통분).

### §13d CAD 실제 반경 기준 사이징 (2026-08-18, 사용자 제공 기하)

도구: `tools/ankle_stopper_detail.py` · 그림 `docs/img/ankle_stopper_detail.png`

![CAD 반경 기준 스토퍼](img/ankle_stopper_detail.png)

*좌: 실제 반경에서의 접촉력. 중: SF 2.0 요구 단면적 — **전단파열이 베어링을 2.6배 지배**한다.
우: 보스 두께 $t$ 대 요구 면 높이 $h = A/t$, 점이 8T 지점.*

⚠**사용자 구두 제공값이며 CAD 확정 전이다** — AB 볼트원 **Ø78**(r 39), RP **Ø46**(r 23),
체결은 **Ø4 SCM 12.9** 나사. 반경이 §13b 권고(≥60 mm)보다 작아 하중이 커진다.

| 스토퍼 | 토크 | 반경 | 접촉력 | 베어링 소요 | **전단 소요** | 8T에서 면 높이 |
|---|---|---|---|---|---|---|
| **AB 크랭크**(지면구동) | 515.1 N·m | 39 mm | **13.2 kN** | 63.8 mm² | **165.9 mm²** | **20.7 mm** |
| AB 크랭크(모터 스톨만) | 60.0 N·m | 39 mm | 1.5 kN | 7.4 mm² | 19.3 mm² | 2.4 mm |
| **RP 짐벌**(지면구동) | 413.0 N·m | **23 mm** | **18.0 kN** | 86.7 mm² | **225.5 mm²** | **28.2 mm** |

- **RP가 r 23이라 r 60 대비 2.6배**를 쓴다. 반경은 $A \propto 1/r$ 로 가장 싼 변수다 —
  Ø92로 키우면 소요 면적이 절반이 된다.
- **재료를 바꾸면 면 높이가 1/3**: SCM440 강재 보스면 AB 20.7 → **7.3 mm**, RP 28.2 → **9.9 mm**.
- **보스 돌출은 상한이 있다**(뿌리 굽힘 138 MPa): AB **≤6.0 mm**, RP **≤8.1 mm**.
  즉 면은 **높게**, 돌출은 **짧게** — "뒤로 길게 뽑는" 방향은 굽힘을 키워 불리하다.
- **나사 뒤 연거리**는 전단파열 계산상 2.1~2.3 mm면 되지만 **관행 최소 2d = 8 mm**가 지배한다
  (알루미늄 피체결부는 3d = 12 mm 권장).

**★Ø4 12.9 나사 1개로는 불가**: SF 2.0 전단 내력이 생크 4.6 kN / **나사골 3.2 kN**이고
나사골이 지배한다 → **AB 5개 · RP 6개**. 나사골이 지배한다는 것은 **전단면에 나사산이 오면
안 된다**는 뜻이므로 **숄더(스트리퍼) 볼트**로 생크가 전단을 받게 하면 5→3, 6→4로 준다.
더 나은 방법은 docs/64 §8d 규율대로 **전단은 다웰핀, 나사는 클램프만** 맡기는 것이다.

**가장 큰 절감은 배치에 있다** — §13a대로 **RP 짐벌 스톱이 먼저 닿게** 하면 AB는 모터 스톨
1.5 kN만 받으므로 **19 mm² · 나사 1개**로 끝난다(현재 13.2 kN·5개 대비 1/7).
...
| 3 | ★**sim ROM ≠ 설계 ROM** — 설계캡 밖 점유 최악 33.0 % / 중앙 16.6 % (§13c) | **최대 미결**. 설계캡을 걸면 스토퍼가 상시 구조부재가 되어 정적 판정 프레임 자체가 무효 |
| 4 | 구세대 정책(flat25p1 P1·no-DR) 포함 포락 | docs/62 프로토콜(수렴·in-DR·멀티시드) 재측정 시 완화 여지 |
| 5 | 스토퍼 **굽힘**은 보스 형상 확정 후 별도 | 단면적 답에는 전단·베어링만 포함 |
| 6 | **CAD 반경 미확정** — §13d는 사용자 구두값(AB Ø78 / RP Ø46) 기준. RP를 Ø92로 키우면 소요 면적 절반 | CAD 확정 후 재계산 |
| 7 | **나사 전단 의존** — Ø4 12.9 × 5~6개가 전단을 받는 구성. 다웰핀 분담 또는 숄더볼트로 전환 검토 | 설계 결정 |
...
## §15 부록 — 요동각 허용치 민감도 · 순수축 분해 (2026-08-16 포크 세션, 기구학 사실이라 §12 하중정정과 무관)

### §15a JS6 요동각 허용치가 크랭크 스톱 문제를 언제 해소하는가

§8의 "크랭크 직사각형 ⊋ ROM 띠" 문제는 **로드엔드 요동각 허용치를 올리면 사라진다**.
방법: 허용치별로 위반영역을 금지셀로 두고, 중립을 포함하는 **최대 ROM 가중 직사각형**을 탐색
(0.5° 격자). 결과:

| JS6 허용 요동각 | 크랭크 스톱 A / B (중립=0) | 남는 ROM | 비고 |
|---|---|---|---|
| 20° (현 스펙) | −27.0~+4.5 / −27.0~+5.0 | **24.8 %** | pitch −6~+30만 생존 |
| 22° | 0.0~+30.0 / −0.5~+32.0 | 29.0 % | pitch −39~0 |
| 25° | −1.0~+46.5 / 0.0~+59.0 | 53.2 % | pitch −50~0 |
| **28°** | **사실상 제한 불요** | **100 %** | 크랭크 스톱(ROM 극값)만으로 충분 |

→ **JS6(커스텀 20deg_flange)가 28°까지 간섭 없이 허용되면 RP 짐벌 스톱 없이도 안전**하다.
현 스펙 20°에서는 §13a의 결론(RP 짐벌 스톱 선접촉)이 유일한 해였음을 재확인.
크랭크각 상관계수는 ROM 박스 내에서 **+0.713**(pitch=공통모드, roll=차동모드 — §15b).

### §15b 순수축 분해 — 기구 커플링의 구조 (중립=0, 단위 °)

| 자세 | φ_A | φ_B | 해석 |
|---|---|---|---|
| 순수 저굴 p−50 r0 | +37.38 | +39.64 | **공통모드**: 두 크랭크 같은 방향 |
| 순수 배굴 p+30 r0 | −24.80 | −24.92 | 〃 |
| 순수 롤 p0 r−20 | +13.88 | −14.05 | **차동모드**: 서로 반대 |
| 순수 롤 p0 r+20 | −13.90 | +14.38 | 〃 |
| 코너 p−50 r−20 | **+46.23** | +30.98 | 공통+차동이 같은 부호로 겹침 → A 극값 |
| 코너 p−50 r+20 | +29.47 | **+49.65** | 〃 → B 극값 |
| 코너 p+30 r−20 | −10.11 | **−41.69** | |
| 코너 p+30 r+20 | **−42.82** | −10.30 | |

저굴/배굴의 비대칭(+37.4 vs −24.8)은 중립 크랭크각(−19.05/−14.24)이 이미 아래로 젖혀져
있기 때문 — 크랭크 하향이 토크레버 확보의 필연이라는 §17g(docs/71) 결론과 일관.

### §15c CAD 파라미터 위생 노트

Fusion 사용자 파라미터에 **레거시 `ankle_AB_r = 65`가 잔존**(2026-08-16 캡처 확인).
실 크랭크는 A 65 / B **62**로 분리(`ankle_A_r`/`ankle_B_r`). `ankle_AB_r`이 아직 B 크랭크
스케치를 구동하고 있으면 B가 65로 오작성된다 — **미사용이면 삭제, 사용 중이면 참조 교체**.
(CAD 실측 B_r = 62.000이라 현재 지오메트리는 정상; 파라미터 참조만 정리 대상.)
