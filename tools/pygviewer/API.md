...
| GET | **`/tx/status`** | - | `{armed, sending, enable, enabled, host, port, last_seq, rate_hz, deadman_age_s, deadman_timeout_s, rejected_count, disarm_reason, last_sent_target, kp_max, kd_max, ttl_ms, arm_token, warnings}` - `sending` is `armed AND the keyboard dead-man is fresh` (can be `false` while `armed` stays `true`, see `/tx/heartbeat`); `arm_token` is generated once per process and must be copied verbatim into the receiver's own `--arm-token` (`bridge/dummy_rx.py` / `bridge/huphy_remote_motion.py`) |
