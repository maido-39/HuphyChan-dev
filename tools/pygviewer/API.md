...
| POST | **`/sync_from_real`** | - | (R12, docs/123 section 10.2/10.2b, 2026-09-04) Sets every actuated joint's manual target = its live real value, for joints with fresh (<0.5s), finite real telemetry - a value outside the contract's `safe_clip` is clamped and reported, never silently smoothed over. Returns `{synced: {joint: value}, clipped: {joint: {real, applied, range}}, skipped: {joint: "no real data"\|"stale"\|"nan"\|"no real data (motor not responding)"}, max_delta_before, sync_token, t}`. The `"no real data (motor not responding)"` skip reason (2026-09-04 bench fix) fires when a joint's `GET /health` diag-based verdict is `dead` (ack/miss/motor_age_ms) even though its position field is still fresh - HUPHY fills a physically disconnected motor's slot with `q=0.0` every frame, indistinguishable from a real reading by value alone; only the diag fields say it never answered. 409 only when NO real telemetry has ever arrived on this process at all (`rx_count == 0`) - most joints having no data (the bench: 1 of 12) is a normal, syncable state, not an error. Required once (dashboard: "0. sync from hardware") before `POST /tx/arm` will succeed - see that row and `pygviewer/hw_sync.py`'s module docstring for the near-miss this fixes |
| POST | **`/tx/arm`** | - | stage 2: refused (409) unless stage 1 is enabled AND the sim is in `manual` mode - policy output must never be transmittable. ALSO refused (409, R12) unless a valid `POST /sync_from_real` covers every joint in the TX `enable` list: a joint never synced is named in the 409; a joint whose LIVE real value has since drifted more than 5&deg; (`hw_sync.DEFAULT_ARM_DRIFT_LIMIT_RAD`) away from its RAW real-at-sync value (never the clipped/applied target - a 2026-09-04 bench fix, see docs/123 section 10.2b: comparing against the clipped value made any out-of-range joint permanently unarmable even standing still) while the manual target itself was never touched is also refused (the real hardware moved out from under an un-touched target) - moving the target yourself after syncing is never blocked, only shown as drift in `GET /tx/status`. On success, the response's `sync.clip_warnings` (non-blocking) lists every synced joint whose real pose sat outside the model's range at sync time, e.g. `L_knee_joint: real 171.2 (deg) is outside the model range [6.0, 114.0] (deg); arming will move it to 114.0 (deg) (57.2 (deg) of travel)` |
...
| GET | **`/tx/status`** | - | `{armed, sending, enable, enabled, host, port, last_seq, rate_hz, deadman_age_s, deadman_timeout_s, rejected_count, disarm_reason, last_sent_target, kp_max, kd_max, ttl_ms, arm_token, warnings, violations_count, sync}` - `sending` is `armed AND the keyboard dead-man is fresh` (can be `false` while `armed` stays `true`, see `/tx/heartbeat`); `arm_token` is generated once per process and must be copied verbatim into the receiver's own `--arm-token` (`bridge/dummy_rx.py` / `bridge/huphy_remote_motion.py`); `violations_count` (A2) is the `side="send"`-only count from `GET /violations`; `sync` (R12) = `{valid, reason, synced_joints, sync_token, sync_time_wall, target_drift_rad, target_drift_deg, target_drift_joint, arm_drift_limit_rad, arm_drift_limit_deg, clip_warnings}` - `valid:false` once telemetry for a synced joint goes stale (>1s), `POST /tx/config` reconfigures, the sim mode leaves `manual`, or the model contract changes; `target_drift_*` is purely informational (how far the CURRENT target has moved from its synced value, for ANY reason) and never blocks arming on its own; `clip_warnings` (2026-09-04) is a list of `{joint, real, applied, range, travel, message}`, one per synced joint currently clipped away from its raw real value - also never blocking |
