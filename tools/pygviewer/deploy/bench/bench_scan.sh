...
TMP=$(mktemp /tmp/huphy_scan_XXXX.yaml)
{
  ...
  echo "    side: left"
  echo "    channel: $CH"
  echo "    interface: socketcan"
  echo "    control_hz: 100.0"
  echo "    motors:"
  for i in $(seq 1 127); do printf "      m%d: {id: %d, model: RS03, kp: 5.0, kd: 0.5}\n" "$i" "$i"; done
} > "$TMP"
ip -details link show "$CH" 2>/dev/null | grep -qE 'state UP|UP,' || { echo "!! $CH is not UP — run: sudo modprobe gs_usb && sudo ip link set $CH up type can bitrate 1000000"; exit 1; }
